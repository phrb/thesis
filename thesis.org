...
- ANOVA (as a special case of the linear model?)
...
\end{align}
#+end_export
The  experimental  design, or  model  matrix, is  an
$n\times{}p$ matrix $\mathbf{X} = [\mathbf{x}_1 \;\dots\; \mathbf{x}_n]^{\top}$,
where  $\mathbf{x}_{1},\dots,   \mathbf{x}_n$  are  experiments.    Running  all
experiments  produces   the  response   vector  $\mathbf{y}  =   [y_1  \;\dots\;
y_n]^{\top}$, where each element is an observation of $f$ subject to measurement
error.

The linear model surrogate for $f$ is written
#+begin_export latex
\begin{align}
...
\end{align}
#+end_export
The resulting vector $\hat{\mathbf{y}} = \hat{f}_{\theta}(\mathbf{X})$ is called
the prediction of  the surrogate model.  The parameter space  for a linear model
is
#+begin_export latex
\begin{align}
...
\end{align}
#+end_export
and the parameter  vector $\hat{\theta}$ will be constructed  using the Ordinary
Least Squares (OLS) estimator, which we discuss in the next section.
...
to  choose  experiments  on  Chapter\nbsp{}[[Design of  Experiments]],  and  how  to
evaluate the  quality of fit for  linear models on Section\nbsp{}[[Quality  of Fit
Metrics]].
...
constant variance $\sigma^{2}$.  We can use the variance of the OLS estimator to
compute confidence intervals for the predictions of the surrogate model. We will
use   the    fact   that   the    variance   of   $\hat{\theta}$    depends   on
$(\mathbf{X}^{\top}\mathbf{X})^{-1}$   when  we   discuss   Optimal  Design   on
Section\nbsp{}[[Optimal Design]].
...
\end{align}
#+end_export
The difference between $\mathbf{y}$ and its projection $\hat{\mathbf{y}}$ is the
model error  $\varepsilon$, orthogonal to  the column space of  $\mathbf{X}$, as
represented  in Figure\nbsp{}[[fig:regression-projection]].   The OLS  estimator can
also  be derived  as a  special case  of the  Maximum Likelihood  Estimator with
...
from  an  $n\times{}p$  design  matrix $\mathbf{X}$  by  using  basis  functions
$h_{0,\dots,m-1}: \mathcal{X}  \mapsto \mathbb{R}$, generating  the $n\times{}m$
model matrix
#+begin_export latex
\begin{align}
  M\left(\mathbf{X} = \begin{bmatrix}
    \mathbf{x}_1 \\
    \vdots \\
    \mathbf{x}_n
...
Equation\nbsp{}\ref{eq:linear-model-surrogate} with $M(\mathbf{X})$.

#+begin_export latex
\begin{align}
  h_{(b)} &= \{\;h_0(\mathbf{x}) = 1,\;h_1(\mathbf{x}) = x_1,
  \;h_2(\mathbf{x}) = x_2)\;\} \label{eq:hb} \\
  h_{(c)} &= h_{(b)} \cup \{\;h_3(\mathbf{x}) = x_1^{2},
  \;h_4(\mathbf{x}) = x_2^{2})\;\} \label{eq:hc} \\
  h_{(d)} &= h_{(c)} \cup \{\;h_5(\mathbf{x}) = x_1x_2 )\;\} \label{eq:hd}
\end{align}
#+end_export
