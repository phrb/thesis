...
We can incorporate  hypotheses over the search space to  fitted surrogates using
the basis  functions we  described when we  discussed linear  regression.  Trend
functions can be added to the  surrogate's predicted mean to leverage underlying
trends  in  data,  that  should  be followed  on  prediction  regions  far  from
measurements.  Figure\nbsp{}[[fig:gp-trends]]  shows GP  surrogates fitted  with and
without  model trends.   Black  circles   represent  six  measurements   of  a
single-input function, and  each colored band represents a model  trend.  We are
