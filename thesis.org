...
This  chapter  discusses  Linear  Regression, a  parametric  method  capable  of
modeling a restricted  class of surrogates for which it  is relatively simple to
interpret   significance,   and   Gaussian  Process   Regression,   a   flexible
nonparametric method capable  of modeling a large class of  surrogates for which
interpreting significance is possible, but costly.

In the  context of this  thesis the  surrogate $\hat{f}_{\theta}$ is  a function
that  maps a  point $\mathbf{x}$  in the  search space  $\mathcal{X}$ to  a real
number.   The  function  $\hat{f}_{\theta}$  depends  on  the  parameter  vector
$\theta(\mathbf{X},  \mathbf{y})   \in  \Theta$,  constructed  from   a  set  of
experiments  $\mathbf{X}$ and  corresponding  observations  $\mathbf{y}$ of  the
objective function $f$.   The process of using the  available data $(\mathbf{X},
\mathbf{y})$ to construct a surrogate model  is called training, or fitting, the
model.  The class  of surrogates we can fit depends  on each method's definition
of  the  parameter  space  $\Theta$. Chapter\nbsp{}[[Notation  and  Search  Spaces]]
presents a more detailed discussion on the notation.

In this chapter we make the  hypotheses that the set of experiments $\mathbf{X}$
and the observations  $\mathbf{y}$ are given, and that we  can no longer observe
the value of $f$ for new $\mathbf{x} \notin \mathbf{X}$.  For the two methods we
discuss, we will  present the model classes that compose  $\Theta$, and how each
method fits a  surrogate to the available data. We  will also discuss strategies
to  evaluate  the  quality  of  and interpret  parameter  significance  for  the
surrogates built by the methods discussed.

*** Linear Regression
We  are  going to  build  a  surrogate  $\hat{f}_{\theta}$  for a  function  $f:
\mathcal{X} \mapsto  \mathbb{R}$ using a fixed  experimental design $\mathbf{X}$
and its associated measurements $\mathbf{y}$. The parameter vector $\theta$ will
be constructed using the Ordinary Least Squares estimator for a linear model.

The experimental  design is an  $n\times{}p$ matrix $\mathbf{X}  = [\mathbf{x}_1
\;\dots\; \mathbf{x}_n]^{\top}$, where  $\mathbf{x}_{1},\dots, \mathbf{x}_n$ are
experiments.  Running all experiments produces a response vector $\mathbf{y}$ of
size $n$, where each element $y_i$ is a measurement of $f(\mathbf{x}_i)$ subject
to measurement error.

Our linear surrogate model for $f$ is
#+begin_export latex
\begin{align}
  \hat{f}_{\theta}(\mathbf{x}) =
  \mathcal{M}(\mathbf{x})\theta(\mathbf{X},\mathbf{y}) +
  \varepsilon\text{.}
\end{align}
#+end_export
The  $\varepsilon$ term  represents  the effects  of  reducible and  irreducible
error,  emerging from  modeling  assumptions and  experimental conditions.   The
function $\mathcal{M}: \mathcal{X}^{n\times{}p} \mapsto \mathbb{R}^{n\times{}m}$
generates  the $n\times{}m$  /model  matrix/  for our  linear  model,  where $m  =
\vert{}\theta\vert{}$.

For  an  $n\times{}2$  experimental  design   $\mathbf{X}$  in  a  search  space
$\mathcal{X} = \{(x_1,  x_2): x_1,x_2 \in \mathbb{R}\}$,  the $n\times{}m$ model
matrix $\mathcal{M}(\mathbf{X})$ corresponding to a linear model with all second
order effects is
#+begin_export latex
\begin{align}
  \mathcal{M}(\mathbf{X}) =
  \begin{bmatrix}
    1 & x_{1,1} & x_{1,2} & x_{1,2}^{2} & x_{1,2}^{2} & x_{1,1}x_{1,2} \\
    \multicolumn{6}{c}{$\vdots$} \\
    1 & x_{n,1} & x_{n,2} & x_{n,1}^{2} & x_{n,2}^{2} & x_{n,1}x_{n,2}
  \end{bmatrix}\text{,}
\end{align}
#+end_export
with $m = 6$, and the $m\times{}1$ parameter vector is
#+begin_export latex
\begin{align}
  \theta(\mathbf{X}, \mathbf{y}) = [\theta_0 \; \dots \; \theta_5]^{\top}\text{,}
\end{align}
#+end_export
