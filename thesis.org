...
Equation\nbsp{}\ref{eq:lin-mod-prob},  but  it  generalizes  much  further.   In
...
The  a  posterior  distribution  representing  our surrogate  model  is  also  a
multivariate normal,  but it  now is  conditioned by the  observed data,  and is
written
#+begin_export latex
\begin{align}
...
\end{align}
#+end_export
It would require an infinite parameter vector  to fit a Gaussian Process for all
...
distribution.   If  we take  100
samples of a 20-dimension Gaussian and unroll each dimension into a single axis,
we end up with the left panel of Figure\nbsp{}[[fig:matern52-50d-mv-samples]].  Each
column of points contains  the 100 values on each dimension  of our samples, and
each  dimension  is  correlated   according  to  the  distribution's  covariance
matrix. The values for one arbitrary sample are marked in larger red dots.
...
#+ATTR_LATEX: :width .75\textwidth :placement [b]
#+CAPTION: Test
[[file:img/matern52_50d_mv_samples.pdf]]
