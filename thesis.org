...
- paragraph: Computing $\hat{\beta}$: OLS
- 1 paragraph: Projection matrix of $\mathbf{y}$, into column space of $\mathbf{X}$
  - Use linear map of columns of $\mathbf{X}$ to describe $\mathbf{y}$
- 1 paragraph: If $\varepsilon$ is normally distributed, equal to Maximum Likelihood Estimator
- paragraph: Natural Extension: Transformations of $\mathbf{X}
- 2 paragraphs: Example on Booth's function (3d?)
...
- paragraph: Train and Test Sets
- paragraph: MSE
- paragraph: Cross Validation, Bootstrapping
- paragraph: Information Criteria for Model Assessment
  - Mallow's C_p and AIC
  - BIC?
****** Interpreting Significance
- OLS standard deviation and Confidence Intervals
- ANOVA (as a modification of the linear model?)
...
This  chapter  discusses  Linear  Regression and  Gaussian  Process  Regression.
Linear Regression is a parametric method  capable of modeling a restricted class
of  surrogates, for  which it  is relatively  simple to  interpret significance.
...
In  the context  of this  thesis, the  surrogate $\hat{f}_{\theta}:  \mathcal{X}
\mapsto  \mathbb{R}$   depends  on  the  parameter   vector  $\theta(\mathbf{X},
\mathbf{y}) \in  \Theta$ constructed using the  pair $(\mathbf{X}, \mathbf{y})$,
where $\mathbf{X}$ is  a set of experiments, and  $\mathbf{y}$ the corresponding
vector of observations of the objective  function $f$.  The process of using the
available data  to construct a surrogate  model is called /learning/  and involves
training, or fitting,  the surrogate model.  The class of  surrogates we can fit
depends  on   each  method's  definition   of  the  parameter   space  $\Theta$.
Chapter\nbsp{}[[Notation and Search Spaces]] the notation in detail.

In this chapter we assume that  an initial pair of $(\mathbf{X}, \mathbf{y})$ is
given,  and  that   we  can  no  longer   observe  $f(\mathbf{x}^{\prime})$  for
$\mathbf{x}^{\prime} \notin  \mathbf{X}$.  For  the two  methods we  discuss, we
will present the model classes represented by each parameter space $\Theta$, and
how each method  fits a surrogate to  the available data.  We  will also discuss
strategies to evaluate  the quality of and interpret  parameter significance for
the   surrogates   built   by   the    methods   discussed.    We   discuss   on
Chapter\nbsp{}[[Design of Experiments ]]how we can construct better distributions of
experiments over  a search space,  and we assume  for now that  $\mathbf{X} \sim
\text{Uniform}(\mathcal{X})$.

*** Linear Regression
We  will build  a surrogate  $\hat{f}_{\theta}$ for  a function  $f: \mathcal{X}
\mapsto  \mathbb{R}$ using  a  fixed experimental  design  $\mathbf{X}$ and  the
associated measurements $\mathbf{y}$.  In a parameter space
#+begin_export latex
\begin{align}
...
\end{align}
#+end_export
the  parameter vector  $\theta$ will  be  constructed using  the Ordinary  Least
Squares (OLS) estimator for a linear model.

The  initial  experimental  design  is  an  $n\times{}p$  matrix  $\mathbf{X}  =
[\mathbf{x}_1  \;\dots\;   \mathbf{x}_n]^{\top}$,  where  $\mathbf{x}_{1},\dots,
\mathbf{x}_n$  are experiments.   Running  all experiments  produces a  response
vector $\mathbf{y}$ of  size $n$, where each element $y_i$  is an observation of
$f(\mathbf{x}_i)$ subject to measurement error.

The  process  of  constructing  $\theta(\mathbf{X}, \mathbf{y})$  with  the  OLS
estimator, which we discuss in the  next section, generates the linear surrogate
model for $f$, written
#+begin_export latex
\begin{align}
  \label{eq:linear-model}
  \hat{y}_i = \hat{f}_{\theta}(\mathbf{x}_i) =
  \mathcal{M}(\mathbf{x}_i)\theta(\mathbf{X},\mathbf{y}) +
  \varepsilon_i\text{,} \;\; i = 1,\dots,k\text{,}
\end{align}
#+end_export
where $\varepsilon_i$ represents the effects of reducible and irreducible error,
emerging from  modeling assumptions and experimental  conditions.  The resulting
vector $\hat{\mathbf{y}} =  [\hat{y}_1 \; \dots \;  \hat{y}_n]^{\top}$ is called
the   prediction   of  the   surrogate   model.    The  function   $\mathcal{M}:
\mathcal{X}^{n\times{}p}   \mapsto    \mathbb{R}^{n\times{}m}$   generates   the
$n\times{}m$   /model    matrix/   for    our   linear    model,   where    $m   =
\vert{}\theta\vert{}$.       In      particular,       $n      =      1$      in
Equation\nbsp{}\ref{eq:linear-model}.
...
Equation\nbsp{}\ref{eq:linear-model}, the resulting prediction vector is
#+begin_export latex
\begin{align}
...
In  general, we  can use  $\hat{f}_{\theta}$  to predict  the value  of $f$  for
$\mathbf{x}^{\prime} \notin  \mathbf{X}$.  For the OLS  estimator, among others,
we can  also determine  prediction standard deviations  and the  significance of
each model term. The interpretation of the

**** Fitting the Linear Model
#+NAME: fig:regression-fitting
#+ATTR_LATEX: :width 0.85\textwidth :placement [t]
#+CAPTION: Fitting a linear regression model to data
[[file:img/regression_example_full.pdf]]
