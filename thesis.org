...
actions\nbsp{}\cite{burnetas1997optimal},  general  other  approaches  exist  to
optimize policies for  general cases that adapt the  gradient-based and function
minimization  methods  we  discuss  in  Chapter\nbsp{}\ref{chap:funcmin}  to  an
objective function that receives a policy and computes the regret.  One of these
approaches    is   the    /Deep    Deterministic    Policy   Gradient/    (DDPG)
