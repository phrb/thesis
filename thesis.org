...
the search space defined by the parameters of a Laplacian GPU kernel.

The kernel was implemented using BOAST\nbsp{}\cite{videau2017boast}, a framework
...
compute each pixel. The factors and  levels defining the search space are listed
in Table\nbsp{}[[tab:gpu_laplacian_factors]].  Considering  only factors and levels,
the size of  the search space is $1.9\times10^5$, but  removing points that fail
at runtime  yields a search space  of size $2.3\times10^4$. The  complete search
space took 154  hours to be evaluated  on /Debian Jessie/, using  an /Intel Xeon
