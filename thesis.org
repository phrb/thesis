...
1. *explore* by choosing the $\mathbf{x}$ with the largest predicted variance in
   $\hat{\boldsymbol{\sigma}}^{2}$;
2. *exploit*  by choosing the $\mathbf{x}$  with the smallest predicted  mean in
   $\hat{\boldsymbol{\mu}}$; or
3. *mix exploitation  and exploration*  by choosing  the $\mathbf{x}$  with the
   smallest lower bound of a 95%  confidence interval constructed using the mean
   predictions       $\hat{\boldsymbol{\mu}}$       and      the       variances
   $\hat{\boldsymbol{\sigma}}^{2}$.
4.  *mix exploitation  and exploration*  by  picking the  $\mathbf{x}$ with  the
...
#+CAPTION: shown in  @@latex: \textcolor{Mahogany}{\textbf{brown}}@@, of an objective function represented by the @@latex: \textcolor{Red}{\textbf{red}}@@
#+CAPTION: line.
