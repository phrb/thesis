...
population $\mathbf{P}_k =  \{\mathbf{x}_1,\dots,\mathbf{x}_s\}$ of size
$s$ to a population $\mathbf{P}_{k +  1}$, which may retain, combine, and modify
elements from $\mathbf{P}$.
...
Instead of progressing from a starting point $\mathbf{x}_1$ toward a final state
$\mathbf{x}_n$, we can employ a set rules to update a starting set of points, or
population, $\mathbf{P}_1 = \{\mathbf{x}_{1,1},\dots,\mathbf{x}_{1,s}\}$ of size
$s$,       toward      a       final       population      $\mathbf{P}_n       =
[\mathbf{x}_{n,1},\dots,\mathbf{n,s}]^{\top}$.   For  simplicity,  we  keep  the
population size constant.  During optimization,  we expect that the distribution
of the population over the search space $\mathcal{X}$ can help exploration
