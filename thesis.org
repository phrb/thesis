...
\begin{align}
  ...
  \mathcal{N}(\boldsymbol{\mu}, \boldsymbol{\Sigma})\text{,}
\end{align}
#+end_export
where the  mean vector $\boldsymbol{\mu}$ is  usually a vector of  $n$ zeros and
...
Learning/\nbsp{}\cite{williams2006gaussian},    Rasmussen   /et.     al/   describe
...
\begin{align}
  ...
  \boldsymbol{\mu}_{\mathbf{X},\mathbf{y}} \in \mathbb{R}^{\vert{}\mathbf{X}\vert{}},\;
...
Table\nbsp{}\ref{tab:kernel-formulas}.

#+begin_export latex
\begin{center}
\label{tab:kernel-formulas}
\begin{tabular}{ll}
\toprule
Kernel & Formula\\
\midrule
Exponential & \(exp\left(-\dfrac{\lVert{}\mathbf{x} -
  \mathbf{x}^{\prime}\rVert{}}{l}\right)\)\\
\addlinespace[1em]
Squared Exponential & \(exp\left(-\dfrac{\lVert{}\mathbf{x} -
  \mathbf{x}^{\prime}\rVert{}^{2}}{2l^{2}}\right)\)\\
\addlinespace[1em]
Matérn & $\dfrac{1}{2^{v - 1}\Gamma(v)}\left(\dfrac{\sqrt{2v}}{l}\lVert{}\mathbf{x} -
...
$(\mathbf{X},\mathbf{y})$,       obtaining       the       distribution       in
Equation\nbsp{}\ref{eq:gauss-posterior},  which is  also a  Gaussian and  can be
written
#+begin_export latex
\begin{align}
  \hat{f}_{\theta}(\mathbf{X}) \sim \mathcal{N}(\boldsymbol{\mu},\boldsymbol{\Sigma})
\end{align}
#+end_export
