...
#+end_example
***** Plot
#+begin_SRC R :results graphics output :session *R* :file "./img/gp_trends.pdf" :width 10 :height 10 :eval no-export
library(ggplot2)
library(dplyr)
library(tidyr)
...
ggplot() +
    ...
    geom_point(data = df,
               ...
               color = "black") +
    ylim(0, 1.0) +
    scale_color_brewer(name = "Predicted Means", palette = "Set1") +
    theme_bw(base_size = 32) +
    theme(axis.title.y = element_text(angle = 0,
                                      vjust = 0.5),
...
We can incorporate hypotheses to the  fitted surrogate using the basis functions
we    described    previously    to    describe    trends    in    measurements.
Figure\nbsp{}[[fig:gp-trends]]  shows GP  surrogates fitted  with and  without trend
models.   The  fits  and  predictions  were computed  using  the  /DiceKriging/  /R/
package\nbsp{}\cite{roustant2018dicekriging}.
