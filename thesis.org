...
- [ ] ANOVA, as a special case of the linear model?
  - partition the sums of squares
  - indicator variables in a linear model
...
outside of $\mathbf{X}$.   In this sense we say  we do not know $f$,  but we can
make hypotheses  about it in  order to construct  surrogate models. For  the two
methods we  discuss, we will present  the hypotheses embedded in  each parameter
space $\Theta$, and how each method fits  a surrogate to the available data.  We
will also discuss strategies to evaluate the  quality of fit of a surrogate, and
to interpret  parameter significance.
...
Section\nbsp{}[[Assessing Quality of Fit of Linear Model Surrogates]].
...
the surrogate's predictions.   Later, we will use the fact  that the variance of
...
**** Assessing Quality of Fit of Linear Model Surrogates
...
surrogates, which used basis function sets
#+begin_export latex
\begin{align}
...
$\mathcal{H}_{(d)}$  from Equation\nbsp{}\ref{eq:hd},  the surrogate  from panel
(c)  has the  smallest MSE  and the  best generalization,  although it  is still
differs from the true $f$.

Figure\nbsp{}[[fig:regression-fitting]]  also  shows  the  adjusted  coefficient  of
determination
#+begin_export latex
\begin{align}
  \bar{R}^{2}  = 1  - \left(\dfrac{n - 1}{n - p - 1} \cdot
  \dfrac{\lVert{}\mathbf{y}  - \hat{\mathbf{y}}\rVert{}^{2}}
      {\lVert{}\mathbf{y} - \bar{\mathbf{y}}\rVert{}^{2}}\right)\text{,}
\end{align}
#+end_export
where $\bar{\mathbf{y}}$ represents the mean  of the observations of experiments
in $\mathbf{X}$.  The  coefficient compares the squared error of  a given linear
model with the  error of the model  that predicts the mean  of all observations,
adjusted by the flexibility introduced by new parameters.
