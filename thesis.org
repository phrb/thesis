...
#+CAPTION: Best points and iterations that found them, for Gaussian Process
#+CAPTION: Regression with Expected Improvementacquisition function (GPR), and uniform
...
#+begin_export latex
\todo[inline]{- Roofline model for Xeon 2630v3 \\
  - Add dgemv performance to it \\
  - Add dgemv tuning (GPR compared to RS)}
#+end_export
*** Summary
:PROPERTIES:
