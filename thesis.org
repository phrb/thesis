...
#+begin_SRC R :results graphics output :session *R* :file "./img/2d_gaussian_uncorrelated.pdf" :width 40 :height 20 :eval no-export
library(ggplot2)
library(patchwork)
library(latex2exp)
library(paletteer)
...
#+RESULTS:
****** Plot
#+begin_SRC R :results graphics output :session *R* :file "./img/radial_basis_kernels.pdf" :width 14 :height 10 :eval no-export
library(ggplot2)
...
ggplot() +
    ...
    scale_color_brewer(palette = "Set1",
                       labels = unname(TeX(unique(df$label)))) +
    scale_x_continuous(expand = c(0, 0)) +
...
#+RESULTS:
****** Plot
#+begin_SRC R :results graphics output :session *R* :file "./img/matern52_50d_mv_samples.pdf" :width 30 :height 10 :eval no-export
library(ggplot2)
library(paletteer)
library(latex2exp)
library(patchwork)
...
p_limits = c(-4, 4)
p_palette = "palettesForR::Topographic"
s_size = 6
...
the context of the magnitude of an effect's estimate.
...
#+CAPTION: Test
[[file:img/2d_gaussian_annotated.pdf]]
