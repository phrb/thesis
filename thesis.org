...
fixed experimental design $\mathbf{X}$,  with observations $\mathbf{y}$. We make
...
the covariance matrix $\boldsymbol{\Sigma}_0$ depends on $\mathbf{X}$.
...
Vector  Machines, and  Ordinary Least  Squares.  They  also describe  how Neural
Networks can be represented by specific GPs in Section /4.2.3/.
...
#+CAPTION: shown on the upper right corner of plot
[[file:img/2d_gaussian_uncorrelated_annotated.pdf]]

In contrast to the linear model discussed in the previous section, the parameter
space  $\Theta$ for  GPs depends  on the  size of  the experimental  design, and
increases  as  more  training  data  becomes  available.   For  a  given  design
$\mathbf{X}$ of size $n$, the parameter space is
#+begin_export latex
\begin{align}
...
The  posterior  distribution   representing  our  surrogate  model   is  also  a
multivariate normal,  but now  it is  conditioned by the  observed data,  and is
written
#+begin_export latex
\begin{align}
...
\end{align}
#+end_export
We will see how to compute this  posterior in the next section. It would require
an infinite  parameter vector  to fit  a Gaussian  Process for  all points  in a
search space $\mathcal{X}$  consisting of a single real number.   In this sense,
Gaussian Process Regression is a nonparametric method.
...
It can  help to build an  intuitive understanding of GPs  to think of them  as a
reinterpretation  of  the  dimensions  of  a sample  of  a  multivariate  normal
...
function,  with  covariance  given  by  the covariance  matrix  used  to  sample
functions.  In  this example we  used the Matérn  kernel, discussed in  the next
section, to compute  the covariance matrix.  This prior  over functions directly
estimates a mean  and its associated variance, for each  value of the surrogate.
We will now discuss how to fit  a GP to observed data, generating predictions of
means and variances conditioned to observations.
...
computes how  strong is  the covariance  between $(\hat{f}_{\theta}(\mathbf{x}),
\hat{f}_{\theta}(\mathbf{x}^{\prime}))$,      based     on      the     distance
$\lVert{}\mathbf{x}                -               \mathbf{x}^{\prime}\rVert{}$.
Figure\nbsp{}[[fig:radial-basis-kernels]]  shows  five   exponential  kernels,  also
...
The  left  panel  of  Figure\nbsp{}[[fig:matern52-50d-fitting]]  shows  100  sampled
...
$v =  \frac{5}{2}$. The  starting mean  vector $\mathbf{\mu}$  is zero.   We see
that,      for      all      radial      basis      functions      shown      in
Figure\nbsp{}[[fig:radial-basis-kernels]], the  covariance between  inputs decreases
as  the distance  between  them increases,  approaching  zero.  Using  different
...
the conditioned posterior after a single observation $(\mathbf{x}, \mathbf{y})$,
computed             using             Equations\nbsp{}\ref{eq:gauss-posterior},
\ref{eq:posterior-mean}, and  \ref{eq:posterior-cov}. The right panel  shows the
posterior  after  observing two  additional  observations.   Note that  all  100
functions sampled  from the conditioned  distributions pass exactly  through the
observations on the center and rightmost panels.
