...
\begin{figure}[t]
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.22]{./img/quali_brazil/heartwall-0-Tesla-K40-Box-eps-converted-to.pdf}
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.22]{./img/quali_brazil/gaussian-0-GTX-980-Box-eps-converted-to.pdf}
...
\begin{figure}[t]
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.22]{./img/quali_brazil/Pathfinder-0-Tesla-K40-Box-eps-converted-to.pdf}
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.22]{./img/quali_brazil/myocyte-0-Tesla-K40-Box-eps-converted-to.pdf}
...
\begin{figure}[t]
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.15]{./img/quali_brazil/MatrixSummary-eps-converted-to.pdf}
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.15]{./img/quali_brazil/Summary-eps-converted-to.pdf}
...
\begin{figure}[t]
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.15]{./img/quali_brazil/RodiniaSummary-eps-converted-to.pdf}
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.15]{./img/quali_brazil/RodiniaSummary_small-eps-converted-to.pdf}
...
\begin{figure}[t]
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.22]{./img/quali_brazil/heartwall-0-Tesla-K40-Best-eps-converted-to.pdf}
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.22]{./img/quali_brazil/gaussian-0-GTX-980-Best-eps-converted-to.pdf}
...
\begin{figure}[t]
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.22]{./img/quali_brazil/Pathfinder-0-Tesla-K40-Best-eps-converted-to.pdf}
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[scale=.22]{./img/quali_brazil/myocyte-0-Tesla-K40-Best-eps-converted-to.pdf}
...
A  Plackett-Burman design  for 15  parameters  contains 20  experiments, and  we
performed 20  repetitions of  each parameter  assignment for  each of  the three
kernels. Figures\nbsp{}[[fig:effects-titanx]]  and [[fig:effects-quadro]] show  the mean
and 95% confidence intervals  for the coefficients of a linear  model fit to the
measurements, and represent the main effect estimates for each factor.
...
\begin{figure}[t]
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./img/gpu-screening/speedups_screening_titanx.pdf}
        \caption{Best solutions found by the autotuner over time for the Heart Wall problem (HWL) in the Tesla K40}
        \label{fig:speedup-titanx}
    \end{minipage}%
    \hfill
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./img/gpu-screening/speedups_screening_quadrom1200.pdf}
        \caption{Best solutions found by the autotuner over time for the Gaussian Elimination problem (GAU) in the GTX 980}
        \label{fig:speedup-quadro}
    \end{minipage}%
\end{figure}
#+end_export

#+ATTR_LATEX: :width 0.9\textwidth :placement [t]
#+NAME: fig:effects-titanx
#+CAPTION: Test
[[file:img/gpu-screening/main_effects_titanx.pdf]]

#+ATTR_LATEX: :width 0.9\textwidth :placement [t]
#+NAME: fig:effects-quadro
#+CAPTION: Test
[[file:img/gpu-screening/main_effects_quadrom1200.pdf]]
...
\begin{figure}[t]
    ...
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./img/gpu-screening/executiontimes_screening_titanx.pdf}
        \caption{Best solutions found by the autotuner over time for the Heart Wall problem (HWL) in the Tesla K40}
        \label{fig:timing-titanx}
    \end{minipage}%
    \hfill
    \begin{minipage}{.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./img/gpu-screening/executiontimes_screening_quadrom1200.pdf}
        \caption{Best solutions found by the autotuner over time for the Gaussian Elimination problem (GAU) in the GTX 980}
        \label{fig:timing-quadro}
    \end{minipage}%
\end{figure}
#+end_export
...
applications applying empirical and automatic tuning techniques. Our results and
clustering  attempts   emphasize  the   importance  of   automatic  optimization
...
High-Performance Computing tasks that require low power and latency increasingly
switch to FPGAs  from GPUs. Hardware specifications are  written using low-level
languages such as Verilog and VHDL, making a challenge for software engineers to
leverage FPGA capabilities.  This became increasingly relevant by the adaptation
of FPGAs for data  centers\nbsp{}\cite{caulfield2016cloud}, a field dominated by
software engineers.
...
The  most  important  sub-problems   to  solve  are  \textit{scheduling},  where
operations are  assigned to specific  clock cycles, and  \textit{binding}, where
operations  are assigned  to specific  hardware functional  units, which  can be
shared between operations.   LegUp\nbsp{}\cite{canis2013legup} is an open-source
HLS   tool   implemented   as   a   compiler  pass   for   the   LLVM   Compiler
...
thought of as\textit{expensive-to-evaluate functions}, we are going to study the
...
\textit{Operation Latency} parameters define the number of clock cycles required
to complete a  given operation when compiled  with LegUp.  \textit{Zero-latency}
...
performed in  a clock  cycle.  \textit{Boolean  or Multi-Valued}  parameters are
...
(\textit{LUT}) measures the number of logic elements and is composed of Adaptive
Look-Up  Table  (ALUTs),  memory  ALUTs,  logic  registers  or  dedicated  logic
registers.   The  \textit{Registers}   (\textit{Regs.}),  \textit{Virtual  Pins}
(\textit{Pins}),  \textit{Block  Memory   Bits}  (\textit{Blocks}),  \textit{RAM
Blocks} (\textit{BRAM})  and \textit{DSP Blocks} (\textit{DSP})  metrics measure
...
function $f(M, W)$ computes a \textit{Weighted Normalized Sum} (\textbf{WNS}) of
...
wall-clock time  metrics. The  \textit{Performance and Latency}  scenario assigns
high weights  to wall-clock  time metrics  and also to  the number  of registers
used.  The \textit{Performance} scenario assigns low weights to area metrics and
...
their   \textit{Weighted  Normalized   Sum}  (\textbf{WNS})   as  described   in
...
autotuner  is to  minimize all  hardware metrics  except for  \textit{FMax}.  To
create a  consistent presentation of  our data in  the heatmaps we  inverted the
ratios  for \textit{FMax}  so that  cell values  less than  $1.0$ always  mean a
...
metric for \textit{Default} and \textit{Random} starts, in the \textit{Balanced}
scenario.   Cell values  less than  $1.0$ mean  that the  \textit{Default} start
achieved  smaller absolute  values that  the \textit{Random}  start.  Values  of
``--''  mean  that the  \textit{Default}  start  could not  find  a  set of  HLS
parameters  that  produced a  valid  output  during  any  of the  $1.5h$  tuning
runs. The \textit{Default} start found better values for most metrics.
...
\textit{Default} start found better values for \textit{Regs} and \textit{Cycles}
for  all  applications.  For  example,   it  found  values  $53\%$  smaller  for
\textit{Regs} and \textit{Cycles} for the \textit{dfmul} application, and $56\%$
and $55\%$ smaller for \textit{Regs}  and \textit{Cycles}, respectively, for the
\textit{mips} application.
...
\textit{Weighted Normalized Sum} (\textbf{WNS}) of hardware metrics, but we were
...
The autotuner found values of \textbf{WNS} $16\%$ smaller for \textit{adpcm} and
\textit{dfdiv},   and  $15\%$   smaller   for  \textit{dfmul}.    Even  on   the
...
autotuner found values of \textbf{WNS} $23\%$ smaller for \textit{dfdiv}, $18\%$
smaller for  \textit{dfmul}, and smaller  values overall in comparison  with the
\textit{Balanced}  scenario.  Regarding  individual   metrics,  the  values  for
\textit{FMax}  were   worse  overall,  with  $14\%$   smaller  \textit{FMax}  in
...
and \textit{BRAM} in \textit{dfdiv} and smaller values overall for \textit{Regs}
and \textit{LUTs}.
...
the  4 scenarios  over 10  runs. Only  the \textit{Weighted  Normalized Sum}  of
...
\textit{adpcm} in the \textit{Performance and  Latency} scenario. The figure also
...
start tuning for \textit{Performance} at the best autotuned \textit{Area} value.
