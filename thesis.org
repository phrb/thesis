...
for \emph{Heart  Wall} problem, also  from Rodinia, and 4  times for one  of the
matrix  multiplication   algorithms  we  implemented.   We   observed  that  the
compilation parameters that  optimize an algorithm for a  given GPU architecture
will not always achieve the same  performance in different hardware, which was a
...
architectures.   Grauer-Gray \emph{et  al.}\nbsp{}\cite{grauer2012auto} autotune
...
target different compilation steps,  namely the \emph{PTX} optimizing assembler,
...
contains 4 optimization strategies for \emph{matrix multiplication} counted as a
single application, 1 vector addition  problem, 1 solution for the \emph{maximum
sub-array    problem}\nbsp{}\cite{ferreira2014parallel},     2    \emph{sorting}
...
represent the third  and first quartile plus and minus  the \emph{inner quartile
range} times 1.5. Finally, circles represent outliers.
...
optimizations   \texttt{--opt-level=0,1,2,3}   are    denoted   by   \emph{-O0},
\emph{-O1},  \emph{-O2}   and  \emph{-O3}.    The  results  for   the  autotuned
configurations are denoted by the keyword \emph{Tuned}.
...
refer the reader to the survey from Nane \emph{et al.}\nbsp{}\cite{nane2016survey} for
