...
optimization method  to autotuning.   The optimization  methods we  discuss have
significant differences  but share the  definitions of basic concepts.   We will
use the  same mathematical notation  to discuss  all methods.  The  remainder of
this chapter  presents the common concepts  and the notation used  in subsequent
chapters, which are summarized in Table\nbsp{}[[tab:notation]].
...
\mathbb{R}$, we  wish to find the  input vector $X =  [x_1,\dots,x_k]^{\top}$ in
the /parameter space/  $\mathcal{X}$ for which $f(X)$ is the  smallest, for all $X
\in \mathcal{X}$. The function $f(X)$ represents, in the autotuning context,
the /performance metric/ we  wish to optimize, such as the  execution time of some
...
assume  we can  take the  opposite  of the  performance metrics  that should  be
maximized.

As an example of an autotuning  problem, consider optimizing the choice of flags
for   a    compiler   with   $k$    flags,   where   a   configuration    $X   =
[x_1,\dots,x_k]^{\top}$ consists  of a  vector of  $k$ /boolean/  values, denoting
whether each flag $x_1,\dots,x_k$ is turned on for the compilation of a specific
application.  To find the compiler  configuration that generates the binary with
the  smallest  execution  time,  we  conduct $n$  /experiments/,  chosen  in  some
arbitrary  fashion,  generating the  /experimental  design  matrix/ $\mathbf{X}  =
...
The function $f: X \in \mathbf{X} \mapsto \mathbb{R}$ represents the performance
metric to be minimized. In this example, evaluating $f(X_i)$ involves generating
the compiler  configuration with the  selected flags,  which is usually  a shell
command or a  configuration file, running the configured  compiler, checking for
compilation errors,  measuring the execution  time of the resulting  binary, and
verifying the correctness of its output.

We can  never observe the /true  value/ of $f(X \in  \mathcal{X})$. Such empirical
tests are always  subject to unknown or uncontrollable effects,  and to inherent
imprecision in measurement.  In practice, we settle for observing and minimizing
$Y  = f(X)  + \varepsilon$,  where  $\varepsilon$ encapsulates  all unknown  and
uncontrollable  effects, as  well as  the measurement  error.  Returning  to the
compiler flag  example, suppose that we  could conduct $n =  2^{k}$ experiments,
measuring  the performance  of the  binaries  generated with  all possible  flag
combinations.  With  such /experimental design/  we would obtain  the measurements
$Y_i = f(X_i) + \varepsilon_i$, for  $i = 1,\dots,2^{k}$.  The /global optimum/ in
this  example would  be the  $X_i$ producing  the fastest  binary, that  is, the
smallest $Y_i$.

Assuming we  are capable of  evaluating $f(X)$ for  an arbitrarily large  set of
experiments $\mathbf{X}$, and that we are not interested in building statistical
models for the  performance of our application, we can  directly optimize $f(X)$
using stochastic descent  methods, or gradient- and hessian-based  methods if it
is possible to compute or estimate first and second order derivatives of $f(X)$.
These function  minimization methods are discussed  in Chapter\nbsp{}[[Methods for
Function Minimization]].

If  we are  not capable  of  directly measuring  $f(X)$, if  it is  unreasonably
expensive or time-consuming to do so, or if constructing statistical performance
models  for  our  application  is  of crucial  importance,  we  can  employ  the
surrogate-based methods discussed in Chapters\nbsp{}[[Statistical Learning]], [[Design
of Experiments]], and [[Online Learning]].   These methods use different strategies to
construct a /surrogate model/
#+begin_export latex
\begin{equation}
\hat{f}(X = [x_1,\dots,x_k]^{\top} \in \mathcal{X}, \theta(X) \in \Theta)\text{,}
\end{equation}
#+end_export
...
$\mathbf{X}  =  [X_1,\dots,X_n]$.   The  /parameter  vector/  $\theta(X)$  in  the
/parameter space/ $\Theta$ will be used  to compute the estimate.  The constructed
surrogate model  $\hat{f}(X, \theta(X))$  can be  used as a  tool to  attempt to
describe and optimize the underlying real function $\hat{f}(X)$, provided we are
able  to  construct a  useful  estimate  of  $\theta(X)$.   We will  discuss  in
Chapters\nbsp{}[[Statistical Learning]] and\nbsp{}[[Design of Experiments]] methods that
use      the      individual      parameter     estimates      $\theta(X)      =
...
Statistical  learning  methods  construct  surrogate models  that  estimate  the
relationships between problem  factors and the target  performance metric. These
surrogates can be constructed without  access to the underlying $f(\cdot)$ using
a set of previously executed experiments, and  can be used to estimate the value
of the function for new output.
...
#+ATTR_LATEX: :align p{.22\columnwidth}p{.2\columnwidth}p{.5\columnwidth} :booktabs t :placement []\footnotesize
|----------------------------------------------------+----------------------+----------------------------------------------------------------------------------------------|
| Symbol                                             | Concept              | Example                                                                                      |
|----------------------------------------------------+----------------------+----------------------------------------------------------------------------------------------|
| $\mathcal{X}$                                      | Input space          | All possible compiler flag assignments                                                       |
| $X = [x_1, \dots, x_k]^{\top} \in \mathcal{X}$     | Input variable       | A specific flags assignment                                                                  |
| $\bf{X} = [X_1, \dots, X_n] \subseteq \mathcal{X}$ | Experimental design  | A set of flag assignments                                                                    |
| $f: \mathcal{X} \mapsto \mathbb{R}$                | Function to minimize | Performance metric, such as the execution time of a binary                                   |
| $Y = f(X) + \varepsilon$                           | Observable quantity  | Measurement of the execution time, subject to error $\varepsilon$                            |
| $\Theta$                                           | Parameter space      | All possible values of the coefficients of a linear model                                    |
| $\theta: X \mapsto \Theta$                         | Parameter vector     | Estimate of the coefficients of a linear model, built using data from an experimental design |
| $\hat{f}: \mathcal{X} \mapsto \mathbb{R}$          | Surrogate for $f$    | A linear model fit used for /predictions/                                                      |
| $\hat{f}(X, \theta(X))$                            | Estimate of $f$ at X | A prediction of execution time for a specific flag assignment                                |
|----------------------------------------------------+----------------------+----------------------------------------------------------------------------------------------|
