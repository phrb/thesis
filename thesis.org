...
for a compiler with $\vert{}\mathbf{x}\vert{}  = p$ flags, where a configuration
...
generates   the  binary   with   the  smallest   execution   time,  we   conduct
$\vert{}\mathbf{X}\vert{} =  n$ /experiments/, chosen according  to some arbitrary
...
#+begin_export latex
\begin{equation}
\mathbf{y} =  \left\{y_i = f(\mathbf{x}_i)  + \varepsilon_i, \; i =  1,\dots,2^{p}\right\}\text{.}
\end{equation}
#+end_export
...
#+begin_export latex
\begin{equation}
\hat{f}(\mathbf{x} = [x_1,\dots,x_p]^{\top} \in \mathcal{X}, \theta(\mathbf{X}) \in \Theta)\text{,}
\end{equation}
#+end_export
where  the function  $\theta :  \mathbf{X}  \in \mathcal{X}  \mapsto \Theta$  is
produced by  some approximation, or /estimation  process/, that uses data  from an
experimental  design  $\mathbf{X}   =  [\mathbf{x}_1,\dots,\mathbf{x}_n]$.   The
/parameter vector/  $\theta(\mathbf{X})$ in the  /parameter space/ $\Theta$  will be
used   to    compute   the   estimate.    The    constructed   surrogate   model
$\hat{f}(\mathbf{x}, \theta(\mathbf{X}))$  can be used  as a tool to  attempt to
describe and optimize the underlying  real function $f(\mathbf{x})$, provided we
are  able to  construct  a  useful estimate  of  $\theta(\mathbf{X})$.  We  will
discuss in  Chapters\nbsp{}[[Statistical Learning]] and\nbsp{}[[Design  of Experiments]]
methods  that  use  the  individual parameter  estimates  $\theta(\mathbf{X})  =
...
\theta(\mathbf{X}))$ that estimate the relationships between problem factors and
the target performance metric, which are encoded in $\theta(\mathbf{X})$.  If we
have no access  to the underlying function $f(\mathbf{x})$,  surrogates can also
be constructed using a set of  previously executed experiments, and then used to
estimate the function values for new output.
...
| $\vert{}\mathbf{x}\vert{} = p$                                           | Search space dimension          | The number of flags to assign                                     |
| $\mathbf{X} = [\mathbf{x}_1, \dots, \mathbf{x}_n] \subseteq \mathcal{X}$ | Experimental design             | A set of flag assignments                                         |
| $\vert{}\mathbf{X}\vert{} = n$                                           | Number of experiments           | Size of the set of flags to compile and measure                   |
| $f: \mathcal{X} \mapsto \mathbb{R}$                                      | Function to minimize            | Performance metric, such as the execution time of a binary        |
| $y = f(\mathbf{x}) + \varepsilon$                                        | Observable quantity             | Measurement of the execution time, subject to error $\varepsilon$ |
| $\Theta$                                                                 | Parameter space                 | All possible values of the coefficients of a linear model         |
| $\theta: \mathbf{X} \in \mathcal{X} \mapsto \Theta$                      | Parameter vector                | Estimate of the coefficients of a linear model                    |
