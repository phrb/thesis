...
*** Methods Based on Derivatives
- Taylor series: derivative information at a point --> approximation information
  around that point
...
\end{align}
#+end_export
where   $\frac{\partial    f}{\partial   x_i}(\mathbf{x})$,    or   equivalently
$f^{\prime}_{x_i}(\mathbf{x})$,  is the  partial  derivative of  $f(\mathbf{x})$
with respect to  variable $x_i$. The vector  $\nabla{}f(\mathbf{x}_i)$ points to
the  direction   of  /greatest   ascent/  in  which,   from  the   perspective  of
$f(\mathbf{x}_i)$, the value of $f$ increases the most.
