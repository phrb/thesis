...
differences, but employ  the same basic concepts.  When  pertinent and possible,
we  will  use the  same  mathematical  notation  to  discuss all  methods.   The
remainder of  this chapter presents the  common basic concepts and  the notation
used in subsequent chapters, which are summarized in Table\nbsp{}[[tab:notation]].
...
\end{equation}
#+end_export
where the  function $\theta :  \mathcal{X} \mapsto  \Theta$ is produced  by some
approximation, or /estimation process/, that uses data from an experimental design
$\mathbf{X}   =   [\mathbf{x}_1,\dots,\mathbf{x}_n]$.   The   /parameter   vector/
$\theta(\mathbf{x})$ in the /parameter space/ $\Theta$ will be used to compute the
estimate.     The     constructed    surrogate     model    $\hat{f}(\mathbf{x},
\theta(\mathbf{x}))$ can be  used as a tool to attempt  to describe and optimize
the underlying real function $f(\mathbf{x})$,  provided we are able to construct
a   useful    estimate   of   $\theta(\mathbf{x})$.    We    will   discuss   in
Chapters\nbsp{}[[Statistical Learning]] and\nbsp{}[[Design of Experiments]] methods that
use    the     individual    parameter    estimates     $\theta(\mathbf{x})    =
