...
,* covariance model :
  ...
  - parameters upper bounds :  29.09091 31.11111
  - variance bounds :  12781.32 1826251
  - best initial criterion value(s) :  -69.89655

N = 3, M = 5 machine precision = 2.22045e-16
At X0, 0 variables are exactly at the bounds
At iterate     0  f=       69.897  |proj g|=      0.39673
At iterate     1  f =       69.705  |proj g|=       0.32801
At iterate     2  f =       69.077  |proj g|=      0.092363
At iterate     3  f =       69.049  |proj g|=      0.021779
At iterate     4  f =       69.046  |proj g|=     0.0013497
At iterate     5  f =       69.046  |proj g|=    2.6322e-05
At iterate     6  f =       69.046  |proj g|=    2.3738e-06
...
norm of the final projected gradient 2.37377e-06
final function value 69.0462

F = 69.0462
final  value 69.046191
converged
...
,* covariance model :
  ...
  - parameters lower bounds :  1e-10 1e-10
  - parameters upper bounds :  29.09091 31.11111
  - variance bounds :  6683.518 944293.5
  - best initial criterion value(s) :  -66.03803

N = 3, M = 5 machine precision = 2.22045e-16
At X0, 0 variables are exactly at the bounds
At iterate     0  f=       66.038  |proj g|=      0.44354
At iterate     1  f =       65.875  |proj g|=       0.28833
At iterate     2  f =       65.748  |proj g|=      0.025947
At iterate     3  f =       65.746  |proj g|=      0.022668
At iterate     4  f =       65.742  |proj g|=      0.012989
At iterate     5  f =       65.741  |proj g|=     0.0025982
At iterate     6  f =       65.741  |proj g|=    0.00023444
At iterate     7  f =       65.741  |proj g|=    4.9078e-06
...
BFGS updates skipped 0
active bounds at final generalized Cauchy point 0
norm of the final projected gradient 4.90785e-06
final function value 65.741

F = 65.741
final  value 65.741006
converged
...
,* covariance model :
  ...
  - parameters lower bounds :  1e-10 1e-10
  - parameters upper bounds :  29.09091 31.11111
  - variance bounds :  2.431172 596.0268
  - best initial criterion value(s) :  -365.3632

N = 3, M = 5 machine precision = 2.22045e-16
At X0, 0 variables are exactly at the bounds
At iterate     0  f=       365.36  |proj g|=       9.9236
At iterate     1  f =       34.207  |proj g|=      0.025028
At iterate     2  f =       34.182  |proj g|=      0.021133
At iterate     3  f =       34.108  |proj g|=     0.0050862
At iterate     4  f =       34.102  |proj g|=      0.001366
At iterate     5  f =       34.101  |proj g|=     0.0001272
At iterate     6  f =       34.101  |proj g|=    3.5854e-06
At iterate     7  f =       34.101  |proj g|=    9.7517e-09
...
norm of the final projected gradient 9.75169e-09
final function value 34.1011

F = 34.1011
final  value 34.101085
converged
#+end_example
...
\begin{align}
  ...
  \sum_{i = 1}^{n}{\left(\left(f(\mathbf{x}_i) + \varepsilon\right) -
    \hat{f}_\theta(\mathbf{x}_i)\right)^2}\text{,} \label{eq:mse-test} \\[1em]
  MSE_{\hat{f}_{\theta}} &= \dfrac{1}{\vert{}\mathcal{X}\vert{}}
  \sum_{i = 1}^{\vert{}\mathcal{X}\vert{}}{\left(\left(f(\mathbf{x}_i) + \varepsilon\right) -
    \hat{f}_\theta(\mathbf{x}_i)\right)^2}\text{, and} \label{eq:mse-train} \\[1em]
...
$f$ is a Gaussian Process (GP), that is,  that it belongs to the class of models
...
relationships and equivalences of GPs to  other methods such as Splines, Support
Vector Machines, and OLS estimation.  They also describe how Neural Networks can
be represented by specific GPs in Section /4.2.3/.
...
In contrast to the  linear model discussed in the previous  section, there is no
need to explicitly  define and explore a parameter space  for GP Regression.  We
can nonetheless use the same framework we used for Linear Regression to describe
a parameter  space $\Theta$ for  GP Regression that depends  on the size  of the
experimental design, and increases as more training data becomes available.  For
a given design $\mathbf{X}$ of size $n$, this parameter space is
#+begin_export latex
\begin{align}
  \Theta = \left\{\;\left(\boldsymbol{\mu}_{\mathbf{X},\mathbf{y}},\;
...
Instead of looking for the best  parameter vector in $\Theta$, we will condition
the  prior Gaussian  distribution to  the observed  data, obtaining  a posterior
distribution.  The conditioned distribution  representing our surrogate model is
also a multivariate normal written
#+begin_export latex
\begin{align}
...
would require  an infinite  parameter vector  to fit a  GP for  all points  in a
search space $\mathcal{X}$  consisting of a single real number.   In this sense,
GP Regression is a nonparametric method.
...
To build an intuitive understanding of GPs it can be helpful to think of them as
a  reinterpretation of  the  dimensions of  a sample  of  a multivariate  normal
distribution.  If we take 100 samples of a 20-dimension Gaussian and unroll each
dimension   into  a   single  axis,   we  end   up  with   the  left   panel  of
Figure\nbsp{}[[fig:matern52-50d-mv-samples]].   Each column  of points  contains the
100 values  on each dimension of  our samples, and each  dimension is correlated
according to the distribution's covariance  matrix. The values for one arbitrary
sample are marked in larger red dots.
...
discuss how to  fit a GP to  observed data, generating predictions  of means and
variances conditioned to observations.
...
useful models with fits  such as those in Figure\nbsp{}[[fig:matern52-50d-fitting]],
we can also produce a GP fit  that incorporates the uncertainty from noisy data.
Figure\nbsp{}[[fig:matern52-50d-fitting-noisy]]  shows the  results of  conditioning
the  prior  distribution under  the  assumption  that the  underlying  objective
function  is  subject  to  measurement error  $\varepsilon  \sim  \mathcal{N}(0,
\sigma^2)$,  as  described   in  Equation\nbsp{}\ref{eq:model-noisy}.   In  this
scenario,   we   write    the   mean   vector   and    covariance   matrix   for
Equation\nbsp{}\ref{eq:gauss-posterior} as
#+begin_export latex
\begin{align}
...
#+ATTR_LATEX: :width \textwidth :placement [b]
...
#+CAPTION: Panel (a) shows noisy measurements of Booth's function,
#+CAPTION: panels (b), (c), and (d) show surrogate predictions for models fit with
#+CAPTION: linear, quadratic, and quadratic plus interactions trend models,
