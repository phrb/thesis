...
policies. Optimal  policies exist  for special  cases when an  MDP has  a finite
number of states and actions\nbsp{}\cite{burnetas1997optimal}.  Other approaches
exist  for general  cases  which  adapt, among  others,  the gradient-based  and
function minimization methods we  discuss in Chapter\nbsp{}\ref{chap:funcmin} to
an objective  function that receives a  policy and computes the  regret.  One of
these    is     the    /Deep     Deterministic    Policy     Gradient/    (DDPG)
...
others\nbsp{}\cite{fursincollective} have  been performed by Grigori  Fursin and
others, and are highly motivating.
