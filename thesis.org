...
a fixed  and given  experimental set  $\mathbf{X}$, or  that we  could carefully
design  $\mathbf{X}$, and  then run  all experiments,  in order  to improve  the
quality  of fit  of our  models. For  the purposes  of the  expositions on  this
thesis,  /Online   Learning/  methods  can   be  understood  as   strategies  to
progressively  decide  which experiments  should  be  performed next,  based  on
uncertainty  regarding the  search  space, modeled  using previous  experimental
data.
...
other surrogates,  by efficiently covering  the search space  with space-filling
designs or targeted sampling to check  the significance of a modeling hypothesis
with optimal design.  These approaches  /explore/ search spaces, seeking to find
out significant  relationships between factors and  response, building surrogate
models that  can aid  in optimization.   In our  DoE approach,  this exploration
stage  is  clearly separated  from  the  subsequent  attempts to  /exploit/  the
identified relationships, where we are  concerned about improving model accuracy
by  fitting  a  smaller  model   with  significant  factors,  finding  the  best
prediction, and restricting the search space of interest.
...
that  maximizes  the sum  of  the  rewards obtained  over  a  time period.   The
...
a configuration  that improves  performance was found  by the  chosen heuristic.
...
$k$ possible actions  $I_1,\dots,I_k$, leading to sequence  of rewards $R_{t,i}.
A sequence of  actions for each instant can  be called a policy, and  we wish to
find  policies, or  strategies  to  construct them,  that  maximize  the sum  of
obtained  rewards  over time.   In  this  section  we  assume that  rewards  are
independent and static  over time, although general MAB solutions  can deal with
dynamic rewards\nbsp{}\cite{auer2019adaptively}.

/Bernoulli/ bandits are a special MAB case when rewards for each of the $k$ arms
are independent Bernoulli  random variables, giving a reward $R_{t,i}  = 1$ with
unknown  probabilities   $P  =   {p_1,\dots,p_k}$,  and   $R_{t,i}  =   0$  with
complementary probabilities.  In this problem the  best reward sum we can expect
over a period  with $T$ instants is $p_{\ast}T$, corresponding  to always making
the  choice that  gives  a reward  with the  highest  probability $p_{\ast}$  in
$P$. In this context, we can write the expected regret of a policy as
#+begin_export latex
\begin{align}
  \label{eq:regret}
  \text{Regret}(\mathcal{I}) = \mathbb{E}\left[p_{\ast}T -
...
Three simple  policies for choosing  an action at  instant $t$ are  exploring by
uniformly choosing  a random action for  instant $t$, exploiting by  picking the
action for which we observed the best  mean reward up to instant $t$, and mixing
exploitation and exploration, by picking the best action so far with probability
$1 - \epsilon$ and a random action  with probability $\epsilon$. The regret as a
function of time, as described in Equation\nbsp{}\ref{eq:regret}, grows linearly
for these policies.

These approaches typically do not explore enough, in the sense that they tend to
always  favor the  action  with  the highest  observed  mean  reward. The  Upper
Confidence Bound (UCB) algorithm\nbsp{}\cite{auer2002using,auer2010ucb} includes
the confidence interval of the observed reward for each action as a component to
decide which choice to make.  At each interval, the algorithm chooses the action
for which the upper  bound of a confidence interval of a  specified level is the
highest.  The confidence levels required should  increase as a function of time,
allowing for more exploration at the  initial stages of optimization, when there
is little knowledge  about the underlying reward distributions,  and pushing for
more exploitation of the best actions later on, when confidence intervals should
be narrower.
