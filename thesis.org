...
performance of an autotuning method based on the Design of Experiments, a branch
of statistics that is not widely  studied or applied in autotuning problems, and
...
La fin de  la loi de Moore et  l'effondrement de la mise à  l'échelle de Dennard
signifient  que l'augmentation  de  la complexité  du  matériel informatique  et
l'optimisation  efficace   de  code  sont  indispensables   pour  maintenir  les
améliorations   exponentielles  des   performances   des  dernières   décennies.
L'optimisation  manuelle de  code n'est  pas adaptée  au nombre  considérable de
configurations de nombreux problèmes d'optimisation de code, mais en plaçant ces
problèmes dans  les cadres  d'optimisation et  d'apprentissage, il  est possible
d'appliquer des méthodes  issues de ces domaines  pour optimiser automatiquement
les  performances  des codes,  un  processus  appelé autotuning.   Les  méthodes
d'autotuning   couramment  utilisées   sont  soit   peu  propices   à  l'analyse
statistique,  comme les  algorithmes génétiques,  soit dépendantes  d'hypothèses
restrictives sur  l'espace de  recherche cible, comme  la Descente  de Gradient.
Dans  cette thèse,  nous développons  et évaluons  la performance  d'une méthode
d'autotuning basée sur  le plan d'expériences, une branche  des statistiques qui
n'est  pas  largement  étudiée  ou  appliquée dans  le  contexte  des  problèmes
d'autotuning,  et qui  aide à  la production  parcimonieuse et  interprétable de
modèles précis.

Nous présentons une série de descriptions et de discussions de diverses méthodes
d'optimisation,  du  point de  vue  de  l'optimisation des  performances.   Nous
décrivons des  méthodes dans les  domaines de  la minimisation de  fonctions, de
l'optimisation  par  modèles  de  substitution,  du  plan  d'expériences  et  de
l'apprentissage  en ligne,  en  nous  concentrant sur  les  propriétés les  plus
importantes pour leur applicabilité aux problèmes d'autotuning. Nous cherchons à
souligner que la recherche d'un  équilibre entre l'exploitation et l'exploration
est implicite  dans l'approche  séquentielle du  plan d'expériences,  et devient
centrale lors  de l'optimisation  d'un espace  de recherche  pour lequel  il est
difficile de formuler des hypothèses.

La principale  contribution de cette  thèse est le développement  d'une approche
d'autotuning transparente et parcimonieuse basée  sur le plan d'expériences, que
nous appliquons à  divers problèmes tels que l'optimisation  de la configuration
des noyaux  GPU et CPU  et la  quantization de bits  à précision mixte  pour les
réseaux  neuronaux.  Nous  présentons aussi  une série  d'évaluations empiriques
d'autres méthodes, sur  des problèmes d'autotuning issus  de différents domaines
du calcul  haute performance,  comme par exemple  des heuristiques  de recherche
coordonnées  par un  algorithme de  bandit  pour optimiser  la configuration  de
compilateurs  pour plusieurs  noyaux  GPU  et FPGA.   Bien  que  il n'était  pas
possible de détecter  et d'exploiter la structure de l'espace  de recherche dans
certains  scénarios expérimentaux,  quelle  que soit  la  méthode choisie,  nous
démontrons comment  les méthodes d'autotuning  basées sur le  plan d'expériences
peuvent  contribuer  à  une  optimisation de  code  interprétable,  efficace  et
effective.
...
otimização,  a partir  da perspectiva  do autotuning.   Descrevemos métodos  dos
domínios de minimização  de funções, otimização baseada  em modelos substitutos,
Desenho de Experimentos e aprendizagem online, nos concentrando nas propriedades
mais relevantes  para a  aplicabilidade a  problemas de  autotuning.  Procuramos
ressaltar que  a busca por  um equilíbrio  entre explorar novas  configurações e
aproveitar  relações já  conhecidas está  implícita na  abordagem sequencial  de
Desenho  de Experimentos,  e se  torna central  ao realizar  a otimização  em um
espaço de busca para o qual a elaboração de hipóteses de modelagem é difícil.
