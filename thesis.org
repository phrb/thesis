...
new_grid = expand.grid(seq(from = -1.0, to = 1.0, length.out = resolution),
                       seq(from = -1.0, to = 1.0, length.out = resolution))
...
df[(df$x < constraint$x_max &
    df$x > constraint$x_min &
    df$y < constraint$y_max &
    df$y > constraint$y_min),
   "z"] = NA
...
p_constrained = ggplot() +
    ...
    geom_point(data = plot_df %>%
               ...
               stroke = optimum_stroke,
               shape = 4) +
    geom_rect(aes(xmin = constraint$x_min,
                  xmax = constraint$x_max,
                  ymin = constraint$y_min,
                  ymax = constraint$y_max),
              show.legend = FALSE,
              fill = "gray94",
    ...
    geom_text(aes(x = (constraint$x_max - constraint$x_min),
                  y = (constraint$y_max - constraint$y_min)),
              size = 12,
              hjust = -0.65,
              vjust = 1.1,
              color = "gray58",
...
****** Plot
#+begin_SRC R :results graphics output :session *R* :file "./img/optima_convexity.pdf" :width 20 :height 10.5 :eval no-export
library(dplyr)
library(ggplot2)
library(paletteer)
library(patchwork)
...
constrained_local_optima = data.frame(x = c(-0.25, -0.45, -0.575),
                                      y = c(0.1, 0.24, 0.34),
...
function with  a single vector  input.  For  a function $f:  \mathcal{X} \mapsto
\mathbb{R}$,   we    wish   to   find    the   input   vector    $\mathbf{x}   =
[x_1,\dots,x_p]^{\top}$ in  the /parameter  space/, or /search  space/ $\mathcal{X}$
for which $f(\mathbf{x})$ is the smallest, for all $\mathbf{x} \in \mathcal{X}$.
The  function  $f(\mathbf{x})$  represents,   in  the  autotuning  context,  the
...
define optimization in the autotuning  context to mean finding the configuration
that minimizes  the target performance  metric. For  the sake of  simplicity, we
assume  we can  use  the opposite  of  the performance  metrics  that should  be
maximized.
...
\end{equation}
#+end_export
The /global  optimum/ in this  example would  be the $\mathbf{x}_i$  producing the
fastest binary, that is, the smallest $y_i \in \mathbf{y}$.
...
#+ATTR_LATEX: :align p{.22\columnwidth}p{.2\columnwidth}p{.5\columnwidth} :booktabs t :font \scriptsize
|--------------------------------------------------------------------------+---------------------------------+----------------------------------------------------------------------|
...
| $\hat{y} = \hat{f}(\mathbf{x}, \theta(\mathbf{X}))$                      | Estimate of $f$ at $\mathbf{x}$ | A prediction of execution time for a specific flag assignment        |
|--------------------------------------------------------------------------+---------------------------------+----------------------------------------------------------------------|
...
from the last section, consisting of  finding the /global minimum/ of a paraboloid
surface.  Using  our notation, the  search space $\mathcal{X}$ for  this example
contains the parameters $x_1, x_2\in [0,1]$

We will look at the search space for the objective functions
#+begin_export latex
\begin{align}
y &= f(\mathbf{x} = [x_1, x_2]^{\top}) = x_{1}^{2} + x_{2}^{2}\text{,} \label{eq:f1}  \\
y &= f(\mathbf{x} = [x_1, x_2]^{\top}) + \varepsilon = x_{1}^{2} + x_{2}^{2} + \varepsilon,
\; \text{with } \varepsilon \thicksim \mathcal{N}(0, \sigma^2)\text{, and} \label{eq:f2} \\
y &= f(\mathbf{x} = [x_1, x_2]^{\top}) + \varepsilon = x_{1}^{2} + x_{2}^{2} + \varepsilon
\text{, with} \; x_1 \notin (0.1, 0.4), \; x_2 \notin (-0.3, 0.4)\text{.} \label{eq:f3}
\end{align}
#+end_export

#+NAME: fig:simple-search-spaces
#+ATTR_LATEX: :width \textwidth :placement [t]
#+CAPTION: Simple search spaces
[[file:img/simple_search_space.pdf]]
