...
Alternatively, it is possible  to model rewards for each arm  as a linear model,
using knowledge about search space structure that can be updated as optimization
progresses.     The     /LinUCB/    algorithm     introduced    by     Li    /et
al./\nbsp{}\cite{li2010contextual}  implements  such   contextual  approach  and
applies it to news recommender systems that adapt to different users.
...
Gradient/ (DDPG) method\nbsp{}\cite{lillicrap2015continuous}  used in the method
we       use        as       a        baseline\nbsp{}\cite{wang2019haq}       in
