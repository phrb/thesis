...
and 95% confidence intervals for the coefficients of a linear model that was fit
to the  measurements, and represent the  main effect estimates for  each factor.
...
#+CAPTION: for target kernels measured on the Titan X GPU
...
#+CAPTION: Main effects estimates and 95% confidence intervals for the
#+CAPTION: \textit{high level} of the 15 factors in the Plackett-Burman design,
#+CAPTION: for target kernels measured on the Quadro GPU
[[file:img/gpu-screening/main_effects_quadrom1200.pdf]]

#+latex: \clearpage

Note  that  because  the  Plackett-Burman   design  matrix  is  orthogonal,  the
confidence intervals for  the effect mean estimates all have  the same size. The
effects we  can detect from  these experiments are small,  except in in  the HWL
kernel  where two  factors  present  large effects,  with  a  few other  factors
presenting significant but smaller effects.  We built new models for each kernel
by selecting the  factors whose mean estimates are negative  and have confidence
intervals that do not cross zero.

We then generated a large set of  points in the CUDA parameters search space and
picked the points for which these  smaller models predict the smallest execution
time. Figures\nbsp{}\ref{fig:timing-titanx} and \ref{fig:timing-quadro} show the
performance of the  points picked by the models and  the baseline configuration,
consisting  of  using only  the  PTX-stage  third level  compiler  optimization,
equivalent to /-O3/.  There was no  difference between the second and third levels
of general compiler  optimization in these experiments, so we  picked /-O3/ as the
baseline.
