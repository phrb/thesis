...
- [ ] Separate new GPUs from older ones
...
improvements.  In  Section\nbsp{}[[Autotuning SPAPT kernels with  Gaussian Process
Regression  and Expected  Improvement]]  we discuss  the  application of  Gaussian
Process  Regression with  Expected  Improvement to  the  same kernel.   Finally,
...
#+CAPTION: Theoretical Roofline model for the /Xeon E5-2630v3/
#+CAPTION: processor\nbsp{}\cite{intel2021xeonAPP,intel2021xeon}.
#+CAPTION: Theoretical peak performance is shown for the memory-bound GEMV kernel.
#+CAPTION: The best point we found during all experiments with the /dgemv3/ SPAPT kernel
#+CAPTION: is shown to be 20 times slower than the theoretical peak.
#+LABEL: fig:xeon-roofline-dgemv
#+ATTR_LATEX: :width 0.65\textwidth :placement [h]
#+RESULTS:
...
Figure\nbsp{}[[fig:xeon-roofline-dgemv]]    shows     the    theoretical    Roofline
model\nbsp{}\cite{williams2009roofline}  for  the   processor  targeted  by  our
experiments, which was the same from our CCGRID paper. The figure also shows the
theoretical peak performance of the /DGEM/  kernel, and the performance that was
achieved by the  best configuration we found for the  /dgemv3/ SPAPT kernel, the
red outlier in Figure\nbsp{}[[fig:gp-rs-dgemv-comparison]].
...
and Experimental Design  for a Laplacian GPU Kernel]], knowing  the global optimum
helps evaluating the performance of an autotuning approach.
