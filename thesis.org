...
\begin{forest}
  ...
  [{Minimize $f: \mathcal{X} \mapsto \mathbb{R}$,\\$Y = f(X = (x_1,\dots,x_k) \in \mathcal{X}) + \varepsilon$},
    ...
    [{Is $f(\theta(X))$ known?},
      draw
      [{Minimization Methods for $f(\theta(X))$},
        draw,
        edge label = {node[midway, fill=white, font = \scriptsize]{Yes}}
        [{Are $\nabla{}f$, $\mathbf{H}f$ known?},
          draw
          [{Stochastic\\Descent},
            draw,
            edge label = {node[midway, fill=white, font = \scriptsize]{No}}
            [{Population\\\textbf{Genetic}\\\textbf{Algorithms}}, draw]
            [{Direct\\\textbf{Nelder-Mead}}, draw]
            [{Single-state\\\textbf{Simulated}\\\textbf{Annealing}}, draw]]
          [{\textbf{Gradient}\\\textbf{Descent}},
            draw,
            edge label = {node[midway, fill=white, font = \scriptsize]{Yes}}]]]
      [{\textit{Statistical Learning}\\Construct surrogate $\hat{f}(\cdot, \theta(X))$},
        draw,
        edge label = {node[midway, fill=white, font = \scriptsize]{No}}
        [{Is $|\theta(X)|$ finite?},
          draw
...
Chapter\nbsp{}[[Autotuning using Function Minimization Methods]].
...
Chapter\nbsp{}[[Applications to Autotuning]] provides  more detailed definitions and
discussions  of  the  applicability,  effectiveness, and  explanatory  power  of
autotuning methods  based on  search heuristics  and statistical  learning.  The
...
* Autotuning using Algorithms for Optimization
