...
choices that compose the path we took across the studies done in this thesis.
...
confidence intervals, is  related to the distribution of experiments  and to the
variance associated with measuring $f$.

Figure\nbsp{}[[fig:conflineffect]]  also  shows  that the  determinants  of  the
matrices $\mathbf{X}^{\transp}\mathbf{X}$,  associated with the designs  in each
panel,  are larger  for designs  with more  distant points.   Combined with  the
confidence intervals for  predictions, this example illustrates  that the choice
of  the design  matrix $\mathbf{X}$  impacts the  variance of  the estimator  of
$\theta$  in  the model  fit  in  Equation\nbsp{}\ref{eq:linmodel-ed}.  We  will
present    a    more   precise    statement    of    this   interpretation    in
...
that maximizes the  cumulative rewards obtained over a period  of time.  Choices
made in expectation of a reward can  be compared to pulling the levers, or arms,
of slot machines in  a casino, which is the analogy that  inspired the MAB name.
In a  discrete time  period with  $T$ instants,  a solution  to the  MAB problem
provides at each instant $t \in \{1, \dots,  T\}$ a choice $I_i$ from a set with
$k$   possible  actions   $A_1,\dots,A_k$,  leading   to  sequence   of  rewards
...
of  obtained rewards  over time.   In this  section we  assume that  rewards are
independent and static  over time, although general MAB solutions  can deal with
dynamic
...
Approaches /2/ and /3/ above are not  guaranteed to explore enough, in the sense
that they tend to always favor the action with the highest observed mean reward,
...
choice to  make.  At each interval,  the algorithm chooses the  action for which
the upper bound  of a confidence interval  of a specified level  is the highest.
The confidence levels required should increase  as a function of time, resulting
in a greedy strategy which exploits the best observed mean rewards and explores,
enforced  by increased  thresholds, ensuring  that we  balance exploitation  and
exploration  and   preventing  missing  opportunities  by   concentrating  on  a
suboptimal arm that produced good results by chance in the past.

The OpenTuner\nbsp{}\cite{ansel2014opentuner}  framework provides an  example in
the autotuning  context, where actions are  choices of search heuristics  to run
for a small number of iterations, and  the rewards are computed as the number of
times  a  configuration  that  improves  performance was  found  by  the  chosen
heuristic, in a sliding time window.   The hypothesis of static and independence
rewards is arguably violated in  the OpenTuner application because, although the
optimizations performed by  each heuristic could be independent if  there was no
communication  between  them,  the  results an  heuristic  finds  are  typically
dependent  on  the previous  regions  of  the  search  space that  it  explored.
Additionally,  the   default  configuration   of  the  framework   implements  a
centralized  manager that  shares  results between  heuristics  and maintains  a
shared global best  configuration.  It is still possible to  implement and use a
different strategy to share resources between heuristics in the framework.
...
corresponding performance metric as a reward.  In this setting, we would have to
...
unevaluated in  high dimensional  search spaces, but  previous knowledge  of the
target problem could be used to reduce the number of configurations.
...
\to A$, determining  which action should be taken at  each state.  Reinforcement
Learning  algorithms  operate are  used  when  the  functions  $P$ and  $R$  are
completely or  partially unknown, and seek  to learn these functions  by testing
different  policies $\pi$.   These algorithms  need to  balance exploitation  of
known  effective  policies,  aiming  to  maximize  the  cumulative  reward,  and
exploration of  new policies, aiming to  learn $P$ and $R$.   Policy choices can
also be different in cases when the maximum duration $T$ is known, in a scenario
said to have  a /fixed horizon/, and  when $T$ is unknown, when  the best choice
would  be  optimize for  a  potentially  infinite  duration with  a  /discounted
reward/.  Figure\nbsp{}\ref{fig:mdp} shows an example  of an MDP with two states
and four actions, with known $P$ and $R$.

Regret for RL algorithms can be modeled  in a similar fashion to the MAB example
of the previous  section, and the strategies we briefly  described for balancing
exploitation and  exploration could  in principle be  applied to  optimizing MDP
policies. Policies  are generally  obtained using  dynamic programming  for both
fixed horizon and  discounted reward cases.  Optimal policies  exist for special
cases    when    an    MDP    has    a   finite    number    of    states    and
actions\nbsp{}\cite{burnetas1997optimal}.

Other  approaches  exist  for  general  cases which  adapt,  among  others,  the
gradient-based    and   function    minimization   methods    we   discuss    in
Chapter\nbsp{}\ref{chap:funcmin} to an objective function that receives a policy
and  computes the  regret.   One  of these  is  the  /Deep Deterministic  Policy
Gradient/   (DDPG)  method\nbsp{}\cite{lillicrap2015continuous}   used  in   the
Reinforcement    Learning     method    we     use    as    a     baseline    in
Chapter\nbsp{}\ref{chap:quantization}.   This  method  is  a  variation  of  the
/Q-learning/ algorithm,  introduced by Watkins\nbsp{}\cite{watkins1989learning},
where a function $Q: S\times{}A \to  \mathbb{R}$ provides the expected reward of
a state-action combination, and is updated using dynamic programming, using data
from observations over time. Finding an optimal policy using the function /Q/ is
hard  in general,  but using  a  Deep Neural  Network to  represent /Q/  enables
optimizing for certain scenarios.
...
observations of  an objective function  $f$. Each experiment  is marked as  by a
green point in Figure\nbsp{}[[fig:ei-gp]], with the  true $f$ marked in red.  As
we  described in  Section\nbsp{}\ref{sec:gpr}, the  fitted surrogate  provides a
vector  $\hat{\boldsymbol{\mu}}$ of  predicted means  for each  $f(\mathbf{x})$,
...
shaded areas, and go to zero at each observation. To decide where to spend the
next measurement, we could:

1. *explore* by choosing the $\mathbf{x}$ with the smallest lower bound of a 95%
   confidence    interval    constructed     using    the    mean    predictions
   $\hat{\boldsymbol{\mu}}$ and the variances $\hat{\boldsymbol{\sigma}}^{2}$;
2. *exploit*  by choosing the $\mathbf{x}$  with the smallest predicted  mean in
   $\hat{\boldsymbol{\mu}}$; or
3. *mix exploitation and exploration* by either using a strategy like the UCB we
   discussed  for MAB  problems, or  picking the  $\mathbf{x}$ with  the largest
   Expected Improvement.

#+NAME: fig:ei-gp
#+CAPTION: A surrogate Gaussian Process, whose mean is shown in @@latex: \textcolor{NavyBlue}{\textbf{blue}}@@, fit to four observations,
#+CAPTION: shown in  @@latex: \textcolor{OliveGreen}{\textbf{green}}@@, of an objective function represented by the @@latex: \textcolor{BrickRed}{\textbf{red}}@@
#+CAPTION: line.
#+CAPTION: The dashed vertical lines show which $\mathbf{x}$ would be chosen for
#+CAPTION: the next observation, if we followed the surrogate's
#+CAPTION: minimum @@latex: \textcolor{NavyBlue}{\textbf{mean}}@@, its
#+CAPTION: @@latex: \textcolor{NavyBlue}{\textbf{mean}}@@ minus its
#+CAPTION: @@latex: \textcolor{DarkOrchid}{\textbf{confidence interval}}@@
#+CAPTION: lower bound, or if we picked the $\mathbf{x}$ that maximizes the
...
predictions.   The point  we would  evaluate by  following the  highest expected
improvement  might  not  be  an   immediate  improvement  on  the  current  best
observation, but it is the choice that will dynamically balance exploitation and
...
suited metric for balancing exploitation and exploration, which can in principle
be applied to all methods relying in statistical models.
...
tackle  the   performance  optimization  of  complex   search  spaces,  enabling
exploitation   of  existing   relationships  between   program  parameters   and
...
Sobol indices  was inconclusive. Applying DoE  methods to this problem  could be
fruitful.
...
Experiments to  their own autotuning  problems.  These and  other collaborations
were  omitted from  this  thesis due  to  time and  space  constraints, but  the
experience  with  applying  DoE  methods   to  problems  of  interest  to  other
researchers aided in  the consolidation of our understanding, in  the writing of
this document, and  in demonstrating the potential of DoE  methods on domains we
did  not  explore  directly  during  this   thesis.   A  complete  list  of  our
collaborations is presented in Appendix\nbsp{}\ref{app:contrib}.
...
differently for  each problem and, with  the exception of the  work presented in
Chapter\nbsp{}\ref{chap:laplacian},  relied on  consensus established  among the
users of a program.

By addressing common issues, these differences expose a set of common steps that
must be taken  for studying most autotuning problems, which  could be abstracted
to produce a standard for  autotuning research. Efforts toward establishing such
a standard\nbsp{}\cite{fursin2021collective}  and using  collective explorations
of     a     search     space     to     aid     further     optimization     by
others\nbsp{}\cite{fursin2014collective} have  been performed by  Grigori Fursin
and others, and are highly motivating.

Producing an  extensive set  of representative, parameterized,  and exhaustively
measured programs for specific hardware would enable comparing the optimizations
produced by  different methods with  known global optima, subject  to controlled
budgets and  software and  hardware environments. Elaborating  such a  set would
present a  considerable challenge for  a single  research project, but  it would
favourably impact  the reproducibility  of autotuning  research.  Still,  due to
characteristics  intrinsic to  the  evolution of  hardware  design that  require
constant increases in complexity, we believe this to be a recurring effort, that
would benefit from social and collective approaches.

Future  work on  the directions  we  believe this  thesis has  pointed to  would
involve  contributing  to  devise  and   improve  collective  solutions  to  the
exploration of search spaces, guided  by statistics well-studied and established
in other  domains. Additionally,  it was  not always  clear to  us that  we were
manipulating  program parameters  at  the most  effective  abstraction level  to
achieve performance improvement.  Thus, we believe that studies of the impact of
parameters  from  different abstraction  levels  should  also be  considered  in
autotuning problems.

We believe  that well specified  sets of exhaustively and  collectively measured
search  spaces,  targeting an  evolving  array  of hardware  architectures,  and
striving  for reproducible  workflows  and results,  would  provide support  for
further advancing the research on automated performance tuning.
...
UGA.       We       have      developed      and      applied       the      DoE
methodology\nbsp{}\cite{bruel2019autotuning}             described            in
Chapters\nbsp{}\ref{chap:laplacian}   and    \ref{chap:spapt},   and   continued
collaborating,  meeting weekly  with *Brice  Videau*  for the  duration of  this
thesis.
...
undergraduate  student at  USP.   During  the nine  months  of  this project  we
continued       the        development       of       our        DoE       Julia
...
research  projects at  the time,  and we  conducted weekly  group meetings  that
helped devise  the best exploration and  modeling strategies for each  case.  We
applied Optimal Design and screening,  to problems such as optimizing parameters
of the Rust compiler  and of the PETSc library\nbsp{}\cite{satish1997efficient},
and  optimizing  hyperparameters  and  feature  selection  of  machine  learning
algorithms    from   the    /Scikit-learn/\nbsp{}\cite{pedregosa2011scikitlearn}
/Python/   library.    We   maintained    a   project   page   with   additional
...
- *Alan Barzilay*: Optimizing Clustering Algorithms for Staying Point Extraction; and
