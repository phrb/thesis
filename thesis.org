...
grad_f = gradient(matrix(df$z, nrow = resolution))
grad_f_noisy = gradient(matrix(df$z_noisy, nrow = resolution))

plot_df = df %>%
    mutate(x_grad = as.vector(grad_f$X),
           y_grad = as.vector(grad_f$Y),
           x_grad_noisy = as.vector(grad_f_noisy$X),
           y_grad_noisy = as.vector(grad_f_noisy$Y))
...
grad_resolution = 6
grad_length = 0.83
picked = unique(plot_df$x)[seq(1, resolution, by = grad_resolution)]
...
wish  to find  the input  vector  $\mathbf{x} =  [x_1,\dots,x_p]^{\top}$ in  the
/parameter space/, or /search space/  $\mathcal{X}$ for which $f(\mathbf{x})$ is the
smallest, for  all $\mathbf{x}  \in \mathcal{X}$.  The  function $f(\mathbf{x})$
represents,  in  the autotuning  context,  the  /performance  metric/ we  wish  to
optimize, such  as the  execution time  of some  application, and  the parameter
space  $\mathcal{X}$  represents  the  set of  possible  /configurations/  we  can
explore,  such as  compiler flags.   Therefore,  we define  optimization in  the
autotuning context to  mean finding the configuration that  minimizes the target
performance  metric. For  the  sake of  simplicity,  we assume  we  can use  the
opposite of the performance metrics that should be maximized.
...
generates  the  binary  with  the  smallest execution  time,  we  conduct  $n  =
\vert{}\mathbf{X}\vert{}$  /experiments/,  chosen   according  to  some  arbitrary
fashion,   generating    the   /experimental   design   matrix/    $\mathbf{X}   =
...
The function $f:  \mathbf{x} \in \mathcal{X} \mapsto  \mathbb{R}$ represents the
performance metric to be minimized.  In this example, evaluating $f(\mathbf{x})$
involves generating the compiler configuration with the selected flags, which is
usually  a  shell  command  or  a configuration  file,  running  the  configured
...
We can never observe the /true  value/ of $f(\mathbf{x})$. Empirical tests of this
nature are always subject to unknown  or uncontrollable effects, and to inherent
imprecision in measurement.  In practice, we settle for observing and minimizing
$y = f(\mathbf{x}) + \varepsilon$,  where $\varepsilon$ encapsulates all unknown
and uncontrollable effects, as well as  the measurement error.  Returning to the
compiler flag example, suppose  that we could conduct $\vert{}\mathcal{X}\vert{}
= n  = 2^{p}$ experiments, measuring  the performance of the  binaries generated
with all  possible flag  combinations.  With such  /experimental design/  we would
obtain the  measurements
#+begin_export latex
\begin{equation}
\mathbf{y} =  [y_i = f(\mathbf{x}_i)  + \varepsilon_i, \; i =  1,\dots,2^{p}]^{\top}\text{.}
\end{equation}
#+end_export
The /global  optimum/ $\mathbf{x}^{\ast}$  in this  example is  the $\mathbf{x}_i$
that  produces the  binary  with  the smallest  execution  time $y^{\ast}$,  the
smallest $y_i \in \mathbf{y}$.

Assuming we are  capable of evaluating $f(\mathbf{x})$ for  an arbitrarily large
set of  experiments $\mathbf{X}$,  and that  we are  not interested  in building
statistical  models for  the performance  of  our application,  we can  directly
optimize  $f(\mathbf{x})$ using  stochastic  descent methods,  or gradient-  and
hessian-based methods if it is possible  to compute or estimate first and second
order derivatives  of $f(\mathbf{x})$.  These function  minimization methods are
discussed in Chapter\nbsp{}[[Methods for Function Minimization]].

If  we  are  not  capable  of  directly  measuring  $f(\mathbf{x})$,  if  it  is
unreasonably  expensive  or   time-consuming  to  do  so,   or  if  constructing
statistical performance models for our  application is of crucial importance, we
can employ the surrogate-based methods discussed in Chapters\nbsp{}[[Statistical
Learning]],  [[Design of  Experiments]],  and [[Online  Learning]].   These methods  use
different strategies to construct a /surrogate model/
#+begin_export latex
\begin{equation}
\hat{f}(\mathbf{x} = [x_1,\dots,x_p]^{\top} \in \mathcal{X}, \theta(\mathbf{X},\mathbf{y}) \in \Theta)\text{,}
\end{equation}
#+end_export
where the function  $\theta : \mathcal{X} \times  \mathbb{R}^{n} \mapsto \Theta$
is  produced  by  some  approximation,  or /estimation  process/,  that  uses  the
...
compute  the estimate.   The  constructed  surrogate model  $\hat{f}(\mathbf{x},
\theta(\mathbf{X},\mathbf{y}))$ can be used as a tool to attempt to describe and
optimize the underlying  real function $f(\mathbf{x})$, provided we  are able to
construct a useful estimate of $\theta(\mathbf{X},\mathbf{y})$.  We will discuss
in Chapters\nbsp{}[[Statistical  Learning]] and\nbsp{}[[Design of  Experiments]] methods
that use  the individual  parameter estimates $\theta(\mathbf{X},  \mathbf{y}) =
[\theta_1,\dots,\theta_m]^{\top}$  to   assess  the  significance   of  specific
/factors/, that is, the individual flags in the compiler autotuning example.

Statistical  learning methods  construct  surrogate models  $\hat{f}(\mathbf{x},
\theta(\mathbf{X}, \mathbf{y}))$ that estimate the relationships between problem
factors   and   the   target   performance  metric,   which   are   encoded   in
$\theta(\mathbf{X},  \mathbf{y})$.   If we  have  no  access to  the  underlying
function  $f(\mathbf{x})$, surrogates  can also  be constructed  using a  set of
previously executed experiments,  and then used to estimate  the function values
for new output.

Chapter\nbsp{}[[Statistical  Learning]]  differentiates between  /parametric/  methods
which make  the hypothesis that  the number $\vert{}\Theta\vert{}$  of estimated
parameters is /finite/, and /non-parametric/ methods which do not, and are thus able
to construct surrogates depending on a large number of estimable parameters.
...
#+ATTR_LATEX: :align p{.23\columnwidth}p{.195\columnwidth}p{.475\columnwidth} :booktabs t :font \scriptsize
|-----------------------------------------------------------------------------------------------------+---------------------------------+----------------------------------------------------------------------|
...
| $\mathbf{x}^{\ast}: y^{\ast} = f(\mathbf{x}^{\ast}) \leq f(\mathbf{x}), \mathbf{x} \in \mathcal{X}$ | Global optimum                  | Flag assignment with smallest execution time                         |
| $\Theta$                                                                                            | Parameter space                 | All possible values of the coefficients of a linear model            |
| $\theta: \mathcal{X} \times \mathbb{R}^{n} \mapsto \Theta$                                          | Parameter vector                | Estimate of the coefficients of a linear model                       |
| $\hat{f}: \mathcal{X} \times \Theta \mapsto \mathbb{R}$                                             | Surrogate for $f$               | A linear model fit used for /predictions/                              |
...
of a function implies the existence of  a single global optimum, whereas lack of
convexity  implies the  existence  of  /local optima/,  as  happens  in our  noisy
functions, and is highlighted in panel (b) of Figure\nbsp{}[[fig:optima-convexity]].
...
This more  abstract example aimed  to illustrate that  it can be  non-trivial to
find the  global optimum of  the simplest of search  spaces, if we  consider the
significant challenges introduced by unknown effects and measurement error.  The
additional challenges introduced by the  time cost to obtain measurements, which
are discussed  in Chapter\nbsp{}[[Design of  Experiments]], guided the  selection of
the  optimization methods  studied in  this thesis.  The next  chapter discusses
...
derivative-based  and   stochastic  or  derivative-free  methods   for  function
minimization.  We discuss the key hypotheses  of these groups of methods, and in
which cases they can be effective for autotuning problems.

For this chapter, we will put aside the idea of using observations to estimate a
parameter  vector  and  construct   a  surrogate  function  $\hat{f}(\mathbf{x},
\theta(\mathbf{X},\mathbf{y}))$.  The  methods discussed in this  chapter do not
construct  a surrogate  function,  and  thus attempt  to  directly optimize  the
objective function $f(\mathbf{x})$. In that sense, because they need to know how
to evaluate it during optimization, they  make the hypothesis that the objective
function  is known.   The  effectiveness  of the  methods  based on  derivatives
requires additional properties of $f(\mathbf{x})$ to be known, such as its first
and second order derivatives, which  imposes additional constraints on objective
functions.

Evaluating derivatives  to determine the next  best step or using  heuristics to
explore a  search space cannot  be done parsimoniously.   Estimating derivatives
when closed-forms  are unknown  and exploring in  the expectation  of leveraging
unknown   search  space   structure   require  a   large   number  of   function
evaluations. We  will discuss Design  of Experiments in  Chapter\nbsp{}[[Design of
...
\begin{align}
  \nabla{}f(\mathbf{x} = [x_1,\dots,x_n]^{\top}) =
        \left[\dfrac{\partial f}{\partial x_1}(\mathbf{x}),
          \dots,
          \dfrac{\partial f}{\partial x_n}(\mathbf{x})\right]^{\top}\text{,}
\end{align}
#+end_export
where $\frac{\partial f}{\partial x_i}(\mathbf{x})$ is the partial derivative of
$f(\mathbf{x})$    with    respect    to     variable    $x_i$.    The    vector
...
$\mathbf{x}_1$.   If  we follow  the  opposite  of  the  gradient at  $x_1$  for
additional points $x_2,\dots,x_n$, each iteration would be
#+begin_export latex
\begin{align}
...
\ref{eq:f1}, and \ref{eq:f2}.  A gradient  descent algorithm would iterate along
the directions  of greatest  descent at  each point and  would easily  reach the
optimum on the search space of panel (a), unless the choice of $\mathbf{x}_1$ is
really  unlucky.  The  descent paths  on  panels (b)  and  (c) would  not be  so
straightforward since  the several local  minima, represented by the  crests and
loops on the contour  lines, can trap the method if  $\alpha_k$ is not carefully
chosen.

The situation  can be  even thornier in  panel (c), where  an unlucky  choice of
$\mathbf{x}_1$  or  $\alpha_k$  can  throw  the descent  path  against  the  top
constraint  border, forcing  the  method to  zigzag along  the  border in  short
steps. This  would happen in the  particular situation of panel  (c) because all
gradient information  guides the descent  across the constraint border,  but the
method cannot cross  it. Gradient descent would  have a hard time  on panels (b)
and (c)  even upon reaching the  basin where the optimum  lies, because gradient
information there is also conflicting due to $\varepsilon$.
