...
I have met many kind people during  this thesis. I thank Alfredo, Arnaud, Brice,
Dejan, and  Sai, who  I consider  friends and  mentors, for  having had  a great
positive impact in  the way I came  to understand research first as  a human and
social endeavour, and second as a precise, pragmatic, and reproducible hope.

Many other friends  have given me opportunity to laugh,  complain, travel, work,
cook, study, talk, and contemplate  life.  With the certainty of incompleteness,
I thank  Marcos, Flávio, Bodo,  Thilo, Andrei,  Danilo, Salah, Adrien,  Tom, and
Christian.

I thank my  mother, my brother, my  father, for showing me  how, especially when
adversity seems overwhelming,  we can find the sparks of  joy, love, and courage
that reignite our strength to continue.
...
factors  and  response,  aiming  to  build surrogate  models  that  can  aid  in
optimization.  In our DoE approach,  this exploration stage is clearly separated
from the subsequent attempts to /exploit/ the identified relationships, where we
become concerned with  improving model accuracy by fitting a  smaller model with
significant factors,  finding the  best prediction,  and restricting  the search
space of interest.
...
possible actions $A_1,\dots,A_k$, leading to sequence of rewards $R_{t,I_i}$.  A
sequence of actions for each instant can be called a policy, and we wish to find
policies, or  strategies to construct  them, that  maximize the sum  of obtained
rewards over  time.  In the discussion  that follows we assume  that rewards are
...
application to autotuning problems.  One way to model an autotuning problem as a
set  of choices  with distinct  rewards  would require  including each  possible
configuration as an action, with a corresponding performance metric as a reward.
In this  setting, we  would have to  run many tests  with each  configuration in
order  to build  a  confidence interval  that we  could  leverage.  Still,  many
configurations  would have  to  remain unevaluated  in  high dimensional  search
spaces, but previous knowledge of the target problem could be used to reduce the
number of  configurations.  Alternatively, it  is possible to model  rewards for
each arm  as a linear model,  using knowledge about search  space structure that
can be updated as optimization progresses.  The /LinUCB/ algorithm introduced by
Li  /et al./\nbsp{}\cite{li2010contextual}  implements such  contextual approach
and applies it to news recommender systems that adapt to different users.
...
Reinforcement Learning  algorithms operate are  used when the functions  $P$ and
$R$ are  completely or partially unknown,  and seek to learn  these functions by
testing different  policies $\pi$. Instead  of directly modeling  and optimizing
$P$ and $R$,  RL algorithms optimize a function $Q:  S\times{}A \to \mathbb{R}$,
in a method  named /Q-learning/\nbsp{}\cite{watkins1989learning}, which provides
the expected reward of a state-action combination.  If optimization is performed
for a  known, finite, and fixed  number of instants  $T$, we can compute  $Q$ by
defining  its recurrence  function  and  filling a  table  of  rewards for  each
...
aiming  to  learn  $P$ and  $R$.   When  the  objective  is to  optimize  for  a
potentially infinite duration,  building a table for dynamic  programming is not
anymore viable, and the function /Q/ must be modeled using different approaches.
