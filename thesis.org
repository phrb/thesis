...
$(\mathbf{X},\mathbf{y})$.    The   function   $\theta  :   \mathcal{X}   \times
...
The constructed surrogate  model $\hat{f}(\cdot, \theta(\mathbf{X},\mathbf{y}))$
can be used  as a tool to  attempt to describe and optimize  the underlying real
function  $f$,  provided  we  are  able   to  construct  a  useful  estimate  of
...
compiler flag  example, the  parameter estimates  $\theta_2,\dots,\theta_m$ will
correspond to one of the $p$ flags.  In this case, $\theta_1$ is the estimate of
the intercept of the  linear model, and $m = p +  1$. Optimization methods using
the Bayesian inference framework, such  as Gaussian Process Regression, which we
discuss   in  Chapter\nbsp{}[[Statistical   Learning]],   associate  a   probability
distribution   to   $\theta(\mathbf{X},\mathbf{y})$,   which   interferes   with
$\hat{f}_{\theta}$ and can be exploited in our optimization context.
...
the  key hypotheses  of these  groups of  methods, and  in for  which autotuning
problems they can be most effective.
...
unless we make an unlucky choice of $\mathbf{x}_1$.  The descent paths on panels
...
\begin{align}
  f(\mathbf{x}_k) \approx f(\mathbf{x}_{k - 1}) \; + \;
    \nabla{}f(\mathbf{x}_{k - 1})^{\top}(\mathbf{x}_k - \mathbf{x}_{k - 1}) \; + \;
    \frac{1}{2}(\mathbf{x}_k - \mathbf{x}_{k - 1})^{\top}\mathbf{H}f(\mathbf{x}_{k - 1})
    (\mathbf{x}_k - \mathbf{x}_{k - 1})\text{,}
\end{align}
#+end_export
where  $\mathbf{x}_k$  is in  the  neighborhood  of  $\mathbf{x}_{k -  1}$,  and
...
polynomial to be  an estimation process in the statistical  sense, because it is
not a strategy to deal with measurement or modeling error.
...
method converges to the  global optimum in a single step if,  in addition to the
previous  hypotheses being  respected,  $f$ is  quadratic  and $\mathbf{H}f$  is
positive definite.   This happens in the following  example with Booth's function
#+begin_export latex
\begin{align}
...
ridges.  Since the search  space in Figure\nbsp{}[[fig:search-space-2]] is discrete,
we would have to perform a considerable number of evaluations of $f$ in order to
estimate  its derivative  at each  step.   Although gradient  descent and  other
derivative-based  methods  can  be  effective and  are  historically  important,
measuring  $f$ is  usually not  cheap for  autotuning search  spaces. Therefore,
minimizing  experimentation  cost is also a strong  concern.
