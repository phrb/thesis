...
$\mathbf{x}$  is the  set of  points within  a maximum  distance $d_{max}$  from
$\mathbf{x}$, and can be defined as
#+begin_export latex
\begin{align}
  N(\mathbf{x}) = \{\mathbf{x}_i \neq \mathbf{x}:
    \; \lVert{}\mathbf{x}_i - \mathbf{x}\rVert{} \leq d_{max}\}\text{.}
\end{align}
#+end_export

A   random   walk   of   length  $n$   starting   at   $\mathbf{x}_1$   produces
$\mathbf{x}_2,\dots,\mathbf{x}_n$,  where   each  $\mathbf{x}_k$  is   a  random
variable with  uniform distribution  over $N(\mathbf{x}_{k - 1})$.
**** Population-Based Methods
*** Summary
** Statistical Learning
*** Introduction
