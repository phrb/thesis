...
autotuning  context, $f(\cdot)$  represents the  /performance metric/  we wish  to
optimize, such  as the execution time,  and $\mathcal{X}$ represents the  set of
possible  /configurations/ we  can explore,  such as  compiler flags.   Therefore,
optimization  in the  autotuning context  means finding  the configuration  that
minimizes the target performance metric.
...
effects, as well as measurement error. Suppose that we could conduct $n = 2^{k}$
experiments, measuring the  performance of binaries generated  with all possible
flag combinations, and  obtain the measurements $Y_i =  f(X_i) + \varepsilon_i$,
for $i = 1,\dots,2^{k}$.  The /global optimum/ in this example  would be the $X_i$
producing the fastest binary, that is, the smallest $Y_i$.

Assuming we are capable of evaluating $f(\cdot)$ for an arbitrarily large set of
experiments $\mathbf{X}$, and that we are not interested in building statistical
models  for  the  performance  of  our application,  we  can  directly  optimize
$f(\cdot)$  using stochastic  descent  methods, or  gradient- and  hessian-based
methods if it is possible to  compute derivatives of $f(\cdot)$.  These function
minimization  methods  are  discussed   in  Chapter\nbsp{}[[Methods  for  Function
Minimization]].  If we are not capable  of directly measuring $f(\cdot)$, if it is
...
Learning]], [[Design of Experiments]], and [[Online Learning]].
