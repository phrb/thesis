...
independent  Bernoulli random  variables, giving  a  reward $R_{t,i}  = 1$  with
...
tend to always favor the action with the highest observed mean reward. The Upper
...
application  to autotuning  problems, since  the best  factor level  choices are
typically interdependent, and  must be done at the same  time, before running an
experiment.
