...
point in ways that  are also inspired by natural processes  such as mutation and
...
During  the optimization process  we expect  that the
distribution of the population over  the search space $\mathcal{X}$ will produce
a more global exploration, while still being capable of escaping local minima.
