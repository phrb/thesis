...
df = bind_rows(df,
               data.frame(x = x,
                          y = 1 + as.vector(k1Fun1Cos(x)),
                          label = c("Cosine")))
#+end_SRC

#+RESULTS:
****** Plot
#+begin_SRC R :results graphics output :session *R* :file "./img/radial_basis_kernels.pdf" :height 10 :width 16 :eval no-export
library(ggplot2)
...
p2 = ggplot() +
    ...
    geom_point(data = plot_df %>% filter(sample == sample_select),
              ...
              show.legend = FALSE,
              size = 5.5) +
    geom_line(data = plot_df %>% filter(sample == sample_select),
              aes(x = x,
                  y = value),
              color = point_color,
              show.legend = FALSE,
    ...
    #scale_y_continuous(limits = p_limits, expand = c(0.01, 0.01)) +
    theme_bw(base_size = 35) +
    ylab(TeX("$\\hat{f}_{\\theta}(\\mathbf{x}_{1},\\ldots,\\mathbf{x}_{20})$")) +
...
***** Fitting Data & Change of Perspective
****** Generate Data
#+begin_SRC R :results output :session *R* :eval no-export :exports results
library(kergp)
library(dplyr)
...
sigma_2_2 = k1FunMatern5_2(obs_1$x_obs, obs_1$x_obs, par = c(range = 1, var = 1))
...
sigma_2_1 = t(sigma_1_2)
sigma_2_2 = k1FunMatern5_2(obs_2$x_obs, obs_2$x_obs, par = c(range = 1, var = 1))
...
*** Gaussian Process Regression
Similarly to  the surrogate model  we built using linear  models, we will  fit a
surrogate  $\hat{f}_{\theta}:   \mathcal{X}  \mapsto  \mathbb{R}$  to   a  fixed
experimental design $\mathbf{X}$ and  observations $\mathbf{y}$. In this section
we make the hypothesis that $f$ is a Gaussian Process, that is, that $f$ belongs
to the class of  models containing all the functions whose values  on any set of
experiments $\mathbf{X}$ can be represented by a single sample of a multivariate
normal  distribution   with  dimension  $d  =   \vert{}\mathbf{X}\vert{}$.   Our
hypothesis over $f$ can be written as the prior probability distribution
#+begin_export latex
\begin{align}
  f(\mathbf{X}) \sim \mathcal{N}(\boldsymbol{\mu}, \boldsymbol{\Sigma})\text{,}
\end{align}
#+end_export
where the  mean vector $\boldsymbol{\mu}$ is  usually a vector of  $d$ zeros and
the  covariance  matrix  $\boldsymbol{\Sigma}$   depends  on  $\mathbf{X}$.

In contrast to the linear model discussed in the previous section, the parameter
space $\Theta$  for Gaussian Processes depends  on the size of  the experimental
design, and  changes as  we increase  the data  we collect.  For a  given design
$\mathbf{X}$, the parameter space is
#+begin_export latex
\begin{align}
  \Theta = \left\{\;\left(\boldsymbol{\mu}_\mathbf{y},\;
  \boldsymbol{\Sigma}\right),\;
  \boldsymbol{\mu}_\mathbf{y} \in \mathbb{R}^{\vert{}\mathbf{X}\vert{}},\;
  \boldsymbol{\Sigma} \in
  \mathbb{R}^{\vert{}\mathbf{X}\vert{} \times{}\vert{}\mathbf{X}\vert{}}\;\right\}\text{.}
\end{align}
#+end_export
Note that  only the mean vector  depends on the observations  $\mathbf{y}$.  The
surrogate model is  a posterior multivariate normal  distribution conditioned by
the observed data, written
#+begin_export latex
\begin{align}
...
As  we perform  experiments the  length of  the parameter  vector $\theta$  of a
surrogate model $\hat{f}_{\theta}$  increases, and it would  require an infinite
parameter vector  to fit  a Gaussian Process  for all points  in a  search space
$\mathcal{X}$  consisting of  a single  real  number.  In  this sense,  Gaussian
Process Regression is a nonparametric method.
...
A perhaps useful intuitive  understanding of a Gaussian Process is  that it is a
reinterpretation  of  the  dimensions  of  a sample  of  a  multivariate  normal
distribution. In  a multivariate Gaussian,  the diagonal of a  covariance matrix
contains  the variances  associated with  each dimension,  and the  off-diagonal
elements   contain   the   covariances   between  each   pair   of   dimensions.
Figure\nbsp{}[[fig:2d-gaussian]]  shows the  impact  of  three covariance  matrices,
which must be symmetric and positive semi-definite.  If we take 100 samples of a
20-dimension Gaussian  and "unroll" each dimension  in a single axis,  we end up
with the left panel of Figure\nbsp{}[[fig:matern52-50d-mv-samples]].  Each column of
points  contains the  100 values  on  each dimension  of our  samples, and  each
dimension is correlated  according to the distribution's  covariance matrix. The
values for one arbitrary sample are marked in larger red dots.
...
$\hat{f}_{\theta}$ evaluated at point $\mathbf{x}_i$.
...
In particular, fitting $\hat{f}_{\theta}$ to data $(\mathbf{X},\mathbf{y})$
