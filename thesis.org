...
Equation\nbsp{}\ref{eq:gauss-posterior} which is also a Gaussian, written
#+begin_export latex
\begin{align}
  \hat{f}_{\theta}(\mathbf{X}^{\prime}) \sim \mathcal{N}(\boldsymbol{\mu},\boldsymbol{\Sigma}) \; \vert{} \; \mathbf{X},\mathbf{y} \sim \mathcal{N}(\boldsymbol{\mu}^{\prime},\boldsymbol{\Sigma})
\end{align}
#+end_export
